...
- [ ] 所有 error 返回使用 `apperr.Wrap(apperr.CodeXxx, msg, err)`
- [ ] 无裸 `error`、`fmt.Errorf`、`errors.New` 在 `internal/` 业务代码中
...
- [ ] PR 描述已 link `internal/xxx/yyy.go` canonical 文件
...
- [ ] R7 lint 通过：gocyclo / nestif（`funlen` 未启用，理由见 ADR-0013 修订记录 2026-07-04）
