...
## W0 故障排查工作流（区别于 W2 编码工作流）

> 排障（"这里为什么坏了"）和编码（"我要新加/改一个功能"）是两种不同的信息需求，不要混用同一套流程。W2 的"阶段 0 上下文锚定"回答的是"该读哪个模块"，前提是已经知道要改什么；排障场景反过来，起点只有一个症状。

1. 先查 `docs/arch/INDEX.md §0 症状索引`。命中 → 直接跳对应文件精读，不要从关键词开始全仓库 grep/通读多个模块。
2. 未命中 → 按现有路径：`grep -ri <症状关键词> docs/arch/decisions/`（是否已有 ADR 记录同类问题）→ 定位可疑 `M_X` 精读。
3. **结论必须有实测依据**：日志时间线、直接调用受影响接口/函数复现、读实际代码路径三者至少占其一，禁止仅凭"改动时间点接近"或"理论上可能"下因果结论（本项目已有一次真实反例：见 §0 索引第 5 行）。
4. 定位到根因后，**强制**在 `docs/arch/INDEX.md §0` 补一行（症状特征 / 归类模块 / 根因类别 / 排查起点），成本控制在一行以内。不补 → 这次排查的经验下次无法复用，等价于没做。
...
1. 任务目标模块的 `internal/<X>/CLAUDE.md`（模块级强约束，如存在）
...
```
已锚定上下文:
- 模块约束: internal/extension/CLAUDE.md
- 标杆:    internal/memory/memory.go
