...
> 适用于 `internal/agent/`（认知核心）、`internal/memory/`（记忆体系）和 `internal/swarm/`（多 Agent 协同）的代码生成约束。
...
四层记忆（`internal/memory/memory.go`）按层隔离：
...
参考 `internal/learning/engine.go` `currentSurpriseIndex()`。
