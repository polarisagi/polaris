...
| 模块 | 标杆文件 | 选定理由 | 复制时关注 |
|------|---------|---------|---------|
| internal/store（SQLite KV） | `internal/store/store.go` — SQLiteStore | 纯 Go 通用 storage 范式 | `protocol.Store` 实现、`apperr.Wrap(apperr.CodeInternal, msg, err)`、MutationBus 串行写约束、WAL + schema 迁移、`fs.ReadDirFS` 注入 |
...
| internal/security/policy | `internal/security/policy/gate.go` — Cedar Three-Layer Gate | 三层 Cedar 主入口 (防退化纵深体系的一部分，删除或绕过需 ADR 级决策) | deny-by-default、forbid-overrides-permit、fail-closed（>10ms 或异常→deny）、连续 10 次失败触发 KillSwitch Stage 1 |
| internal/security (防退化纵深体系) | `internal/security/taint/` + `policy/gate.go` + `network/` | 污点五级+Cedar+KillSwitch+SSRFGuard 标杆 (领先设计防退化，删除或弱化需 ADR 级决策) | TaintPropagation、Cedar Three-Layer、KillSwitch、SSRFGuard |
...
| internal/agent/fsm | `internal/agent/fsm/state_machine.go` — StateMachine | 状态机持有控制流（HE-Rule-5 物理落地） | Transition Guard/Effects 双段拆分、`LLMFillEffect` vs `DeterministicEffect`、`replanCount`+`eventSeq` replay key、S_INTERRUPT/S_SUSPENDED 12 态扩展、`UserInterrupt <200ms SLO` |
...
| internal/extension/skill（内存注册） | `internal/extension/skill/skill.go` — RegistryImpl + SelectorImpl + SkillExecutor | 内存技能注册 + 启发式选择 + Python ContainerSandbox 执行 stub | 直接存 `protocol.SkillMeta`、"skill:" 前缀强制、HMAC-SHA256 `SignatureValid` 准入、Selector 评分公式（Cap40+Cx30+Pass20+Lat10） |
...
| internal/tool/builtin (记忆写入) | `internal/tool/builtin/memory_tools.go` — MemoryWriteTools | 主动式即时记忆写入标杆 (领先设计防退化，删除或绕过需 ADR 级决策) | 即时写入/覆盖、HybridRetriever 检索、SandboxTier=InProcess |
| internal/swarm/orchestrator | `internal/execute/orchestrator/sqlite_blackboard.go` — Blackboard + TaskEntry | M8 多 Agent Blackboard 范式 | `TaskStatus` 单调状态机、`Version atomic.Int32` 防 ABA、CAS 认领、LeaseTTL=60s / Heartbeat=15s±5s / Reaper=1s |
...
| internal/automation | `internal/automation/resource_governor.go` — ResourceGovernor | M13 三级降级资源治理 | 内存/CPU 探针、L1/L2/L3 阈值（1.5GB / 1.0GB / 512MB）、`sync.Cond` 让出准入、并发上限抢占式管理 |
...
| internal/gateway/server（HTTP Handler） | `internal/gateway/server/sysadmin/channelsadmin/channels_crud.go` | HTTP Handler 四段式范式 | 输入解析→业务方法→错误 Warn→JSON 编码；SQL 不内嵌 handler；`slog.Warn("server: xxx failed", "err", err)` |
...
| internal/gateway/authcontext | `internal/gateway/authcontext/contextref.go` — ContextRefExpander | 请求预处理 + 鉴权上下文范式（2026-07-08 合并裂脑实现后确立）| `@file`/`@url`/`git:` 引用展开顺序（基础校验后、SlashCommandRouter 前）、`AuthContext` context.Value 注入/提取、匿名兜底 fail-safe |
