...
// @producer: internal/store/(SQLite / SurrealDB-Core 引擎适配器)
...
- 唯一错误类型：`pkg/apperr/apperr.go` 的 `Error`
- 禁止裸 `error`、`fmt.Errorf`、`errors.New` 在业务代码中
- 错误传播：`return apperr.Wrap(apperr.CodeInternal, "用户可读的描述", err)`
- 代码常量：`CodeInternal`, `CodeNotFound`, `CodeForbidden`, `CodeResourceExhausted`（完整 16 个见 `pkg/apperr/apperr.go`）
- 降级路径：`if err != nil { log.Warn("降级描述"); fallback() }`，不 panic

参考 `internal/automation/resource_governor.go` 的 `apperr.Wrap` 用法。
...
- 不允许 `init()` 函数在 `internal/` 业务包下（仅 `cmd/` 入口允许）
...
- 禁止：`context.Background()` 在 `internal/` 业务路径（仅 `cmd/`/test/cleanup goroutine 允许）
