...
L0 基础设施层 → L0 不可引用 L1/L2/L3
  ...
  internal/sysinfo/    系统信息采集（硬件探针；2026-07-07 从 sysmgr/ 迁出）
  internal/downloader/ 通用资源下载（HTTP/Git；2026-07-07 从 sysmgr/ 迁出）
  internal/config/     配置加载
  internal/protocol/   跨模块共享类型与接口契约（所有层均可引用）
  internal/lint/       CI 静态扫描规则
```

> **注意**：`internal/bootstrap/` 为跨层初始化编排器（Bootable + DependencyMap + Kahn 拓扑排序，四阶优雅关停）。**已定义未接线**的自动编排契约（见 `ARCHITECTURE.md §8.2` / `ADR-0088 §决策七`），生产装配物理落点为 `cmd/polaris/boot_*.go`。不属于 L0~L3 业务层，不可被任何 L0~L3 包反向引用。类比 `pkg/` 的角色但仅限初始化生命周期。
