...
| 07 | `07-Reference-Implementation.md` | 标杆代码索引（每功能域 1 份） | 写新代码前 |
| 01 | `01-Go-Code.md` | Go 结构/错误/并发 | 写 Go 时 |
| 02 | `02-Rust-FFI.md` | Rust FFI 边界 / purego ABI / 内存安全 | 碰 `rust/substrate/` |
| 03 | `03-Agent-Pattern.md` | FSM/事件/Memory/Skill | 改 `internal/agent/` `internal/memory/` `internal/swarm/` |
...
| 09 | `09-LLM-Agent-Production.md` | LLM Agent 生产工程化要求、A 系列陷阱目录、P 系列原则、RAG 专项检查清单 | 写任何 Agent/LLM/Tool/RAG 相关代码前 |

`internal/<X>/CLAUDE.md` 进入目录时手动读取（Claude Code 自动注入，其他 AI 工具需手动加载）。
...
4. 规范冲突 → 00 优先，其次编号小者优先；**规约文档（`docs/arch/` + `docs/specs/`）与代码冲突→以规约为准并修代码**
