...
EvalCase:
  ...
  Input map[string]any / Expected map[string]any
  Level EvaluatorLevel (Level1Assert…Level5Human)
  Severity: P0(阻塞) | P1(警告) | P2(记录)
  Source string / Tags []string / Config map[string]any
  BehaviorType: tool_call_sequence | semantic_quality | format_compliance | safety_boundary
...
**TrajectoryRecorderImpl** 构造时需注入 `protocol.Store`（`NewTrajectoryRecorder(store)`）；`Record()` 扫描 `events:session:{id}:` 前缀，按 Type 分流：`llm_call/inference_request` → LLMCalls，`action_pending/action_done/tool_call` → ToolCalls，其余 → StateTrans。`RunnerImpl.RunReplay` 直接扫描 Store 验证 offset 连续性和零新 LLM 调用。
...
`RunnerImpl` 及评测器接口定义分布于 `internal/eval/` 根目录（旧体系）与 `internal/eval/harness/`（新体系，BehaviorType/FalsifiabilityScore）。当前两套并存，旧体系待清理并统一至 `eval/` 子包。L4 LLMJudge 已同步实现（`internal/eval/harness/`），15s 超时，JSON 提取，支持 nil Provider 时退化为字符串匹配；L5 为人工校准通道，每两周抽样。`BlockDeploy = P0PassRate < 1.0`，`WarnDeploy = P1PassRate < 0.8`。

**`internal/eval/harness/` 核心文件**：`runner.go`（L1-L5 Evaluator + RunnerImpl）、`sampling_monitor.go`（连续采样监控，1% 流量滑动窗口退化检测）、`founding_anchor.go`（FoundingAnchor 演进保护，**P1：含 `globalDriftScore`/`SelfImproveFreeze` 包级全局变量，待 upgrade-12 消除**）、`shadow_executor.go`（影子执行对比）、`synthetic_adapter.go`（L3→L2 转换，SyntheticCase→EvalCase）、`store.go`（SQLiteEvalStore）。
...
流程：IncidentPayload（含 Input/Expected/taint_level）→ PII 脱敏（PIIDetector 接口）→ 写 `pending_review` 分区（Level4LLMJudge, SeverityP0）。高污点（taint≥3）或 needs_human_audit=true 的事件拒绝自动转换，要求人工审核。`ReviewAndPromote()` 提供 HITL 审批入口，确认后迁移至 validation 分区。
...
ShadowExecutor/ShadowDiff/RegressionDetector 实现见 `internal/eval/harness/`。ContinuousSamplingMonitor ✅ 已实现于 `internal/eval/harness/`，滑动窗口 100 条 + 10min 定时检测 + 退化归因（Internal/External/Mixed），`onDegradation` 回调由 M9 注入。
...
实现：`internal/eval/harness/`（SQLiteEvalStore）。基于 `protocol.Store` KV 接口（Scan/Get/Put/Delete）。提供 GetTrainingCases / GetValidationCases（Ed25519 签名校验）、PutCase（含分区校验）、PromotePendingCase（pending_review→validation 迁移）、GetPassRateAvgSince（时间窗口通过率统计）。
...
| 接口定义 | Evaluator/EvalResult/EvalCase/TrajectoryEvent | internal/eval/harness/（新体系）；根目录旧文件 ROUND21 清理中 |
