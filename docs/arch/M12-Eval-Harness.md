...
**⚠️ 组件状态更新**：`shadow_executor.go`（原被误删）已于后续版本重新实现，基于异步 EventLog 回放（ADR-0038），并提供 `ShadowExecutor` 给 M9 ProgressiveRollout 使用。
...
**✅ ShadowExecutor 已实现**。通过 `ShadowExecutor.RunReplayBatch` 从 `events` 表定期抓取 `llm_call` 记录，对新候选配置进行回放打分，并生成 ConfirmShadow 或 Rollback 信号（实现于 `internal/eval/analysis/shadow_executor.go`，由 ADR-0038 决策选用异步 EventLog 回放）。

ContinuousSamplingMonitor ✅ 同样已实现于 `internal/eval/harness/`，滑动窗口 100 条 + 10min 定时检测 + 退化归因（Internal/External/Mixed），`onDegradation` 回调由 M9 注入——该项与 ShadowExecutor 相互独立，不受上述缺口影响。
