...
<!-- §跳读: 0-bis:6 职责 / 0-ter:18 不变量速查 / 1:31 EvalCase / 2:58 Evaluator5层 / 3:78 轨迹录制 / 4:91 Runner / 4.1:112 开放基准适配器 / 5:124 Suite分区 / 6:157 IncidentToEval / 7:163 AutoBootstrap / 8:173 影子执行 / 9:181 连续采样 / 10:197 增量快照 / 11:209 回归检测 / 12:217 集成回放 / 13:221 InvariantTestSuite / 14:234 EvalStore / 15:238 闭环 / 17:244 (SOFT)降级 / 16:263 RedTeam常态化对抗 / 17-bis:280 已知Bug修复记录 / 18:286 依赖 -->
## 0-bis. 职责边界
...
**写侧生产接入（2026-07-14，见 ADR-0048）**：`ContinuousSamplingMonitor.MaybeSampleAndScore`（`internal/eval/analysis/sampling_scorer.go`）按 `samplingRate` 概率对一轮生产问答异步打分并回灌 `RecordSample`；接入 `ChatHandler.SampleAndScoreReply`（`internal/gateway/server/chat/sessions_helpers.go`），覆盖全部 4 条生产 assistant 回复路径（交互式 SSE / 频道 webhook / cron 定时任务 / workflow 步骤）。打分用 Provider 复用 `Registry.PickProvider("default")` → `("general")` 兜底链，与系统提示词组装同一取用方式，不新增 Provider 解析路径。抽中样本在独立 goroutine 中执行，不共享调用方请求 ctx 生命周期。
