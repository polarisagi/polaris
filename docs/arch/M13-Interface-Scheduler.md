...
**触发流**：
```
trigger_type=cron   → cronTickWorkflows(60s) → executeWorkflow → 顺序调 runWorkflowStep
trigger_type=manual → POST /v1/workflows/{id}/trigger → executeWorkflow
```
...
GET    /v1/workflows/{id}         获取工作流详情（含步骤列表）
PUT    /v1/workflows/{id}         更新（steps 字段非 nil 时全量替换步骤）
