...
> **§跳读**: 0-bis:6 职责 / 0-ter:21 不变量速查 / 1:34 对外接口 / 2:286 对内调度 / 3:387 MCP / 6:398 (SOFT)降级 / 7:423 跨模块契约 / 8:440 Web UI 规约 / 8.6:插件聚合市场DB+流 / 8.7:自动化中心DB+流+工作流 / 8.8:电脑操控权限+Preferences
## 0-bis. 职责边界
...
| inv_M13_06 | 所有出站请求经 EgressGateway → M11 SafeDialer.DialContext 完整五阶段 SSRF 防护 | M13 §1.2.2 EgressGateway ✅ 已实现 |
...
`internal/gateway/`（SlashCommandRouter）—— 在 SSE 层拦截用户输入（provider 选取后、LLM 推理前），不消耗 TokenBudget，不进入 Agent FSM。
...
RateLimiterMiddleware:
  ...
  配额(per fingerprint+client_type): CLI 50/s; WebUI 30/s; A2A 30/s; WS 5/s; /_admin/ 10/s
...
ResourceGovernor（`internal/gateway/`，**阈值 SSoT: `spec/state.yaml §thresholds.memory_pressure`**，与 M3 OSMemoryGuard 共享同一配置节，禁止独立硬编码）:
