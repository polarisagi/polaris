...
> ⚠️ **当前实现缺失**：`PipelineImpl.Ingest` 无任何摘要生成逻辑，摘要索引不存在（P1 缺陷）。
...
> ⚠️ **当前实现缺失**：IncrementalIndexer 完全缺失。`SyncScheduler` 能驱动 Ingest/Delete，但无 Hash 对比增量检测逻辑，无 `document_sync_conflicts` 表处理，无 Tombstone + Compaction 原子事务（P1 缺陷）。
...
> ⚠️ **当前实现缺失**：代码中未找到 `RegisterOutboxHandlers` 调用，GraphBuildWorker 无法被 Outbox 驱动触发（P1 缺陷）。
...
| PipelineImpl 结构化解析（6 阶段完整版） | — | ⚠️ 严重缩水：仅双换行分块，无 Markdown/PDF/代码解析器，无多级摘要，无 Embedding，无 SurrealDB-Core 索引 |
| StructuredNavigator + QueryPlanner | — | ⚠️ 完全缺失 |
| IncrementalIndexer（Hash 对比增量检测） | — | ⚠️ 完全缺失 |
| Outbox RegisterOutboxHandlers | — | ⚠️ 缺失：GraphBuildWorker 无法被 Outbox 驱动 |
