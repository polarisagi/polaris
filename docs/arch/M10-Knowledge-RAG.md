...
> **当前实现**：`rag_impl.go` 已实现父子双存，`chunkDocument` 采用语义边界切分（✅ 已修复）：先按 `\n\n` 切段落合并为 ParentChunk（≤1000 runes），再按中英文句子结束符（。！？；/ `.` `!` `?` + 空格）切 LeafChunk（≤250 runes），无边界时兜底字符切；结构化解析器（goldmark/pdfcpu/tree-sitter）仍 **[计划中，确认未开发]**（`go.mod` 无对应依赖，代码里是启发式段落/句子切分 + `func `/`class ` 前缀匹配代替 AST 解析）。**多级摘要生成已实现，见 §1.6**，不再是这条注释里的缺口。
...
> ✅ **已实现**（本条注释此前描述的"`PipelineImpl.Ingest` 无摘要生成逻辑"已过时）：三级 LLM 摘要落地于 `internal/knowledge/rag_impl.go`（`buildSummaryTree`，第129-219行，段落/章节/文档三级调用 `provider.Infer`，对应 `009_rag_chunks.sql` 的 `chunk_type='para_summary'/'chap_summary'/'doc_summary'`），由 `cmd/polaris/boot_knowledge.go` 接线进生产启动路径。另有 `internal/knowledge/graphrag/summary_gen_handler.go`（Outbox 异步摘要处理器，监听 `rag_doc_summary_needed` 事件）和 `internal/knowledge/graphrag/community_summarizer.go`（`CommunityGenerativeSummarizer`，对 Leiden 社区检测结果生成摘要+关键词）——后者是比本节"按文档层级摘要"更成熟的 GraphRAG 风格社区摘要，属于同一需求的升级实现。
...
| 代码 | 按 func/class 边界 | 完整函数体+文件/包路径 | import block 独立索引 | ✅ CodeChunker；tree-sitter AST 计划中 |
| PDF | 布局感知段落+表格单独提 | 完整段落+章节标题+页码 | 图片提取alt text | ⚠️ pdfcpu 计划中；当前 PlainTextChunker 兜底 |
...
- **<8GB VPS（FeatureDeepRAG 不可用）**：HybridRetriever（BM25+Vector RRF）→ ContextExpander（LeafChunk 扩展 ParentChunk + 兄弟块，纯 DB 查询）
...
| GraphTraverser（BFS 图遍历）| `internal/knowledge/` | ✅ 已实现；BFS depth=2，每节点≤20边，总节点≤200，种子实体 LIKE 匹配，关联 chunk 按深度衰减评分 |
