...
| XR-08 | **日志规范**：全项目唯一日志器 `log/slog`（Go 1.21+ stdlib）。格式 `slog.{Level}("subsystem: message", "key", val)`。必选 key：`"err"` (error 值) / `"agent"` (agentID) / `"task_id"` / `"session"` / `"provider"`。禁止：`fmt.Printf` / `log.Printf` / `fmt.Println` 在 internal/ 业务路径；吞 error 的 Warn（必须带 `"err"` key）。级别：Error=需人工介入 / Warn=可降级 / Info=启动·关闭·状态变更 / Debug=高频热路径（≤每请求级别）。 | 全模块 |
| XR-09 | **LLM 调用规范**：所有推理通过 `protocol.Provider.Infer/StreamInfer`，由 `inference.ProviderRegistry` 路由。禁止：任何 internal/ 直接构造 HTTP 请求调 LLM API（同时违反 XR-06）；API Key 存全局变量或结构体字段（必须 `credentialFn func() string` JIT 拉取，使用后 `subtle.ConstantTimeCopy + memclr` 清零）。`InferRequest` 必须声明 `ReasoningEffort` 字段。Canonical: `internal/llm/adapter/anthropic.go`。 | M1→M4,M9,M10 |
| XR-10 | **工具/技能/插件执行规范**：①工具执行唯一入口 `protocol.ToolRegistry.ExecuteTool`（PolicyGate→RateLimit→Sandbox→Result）；②脚本技能唯一入口 `protocol.SkillExecutor.ExecuteSkill`（Rust 沙箱/ContainerSandbox，npx tsx 执行）；③内置工具直接 Go 调用（完全信任，不走沙箱）；④MCP 工具必须通过 MCP Manager 注册到 ToolRegistry，禁止直接 RPC 调用；⑤禁止 `os/exec.Command` 在 internal/agent/、internal/swarm/、internal/eval/、internal/gateway/ 中（仅 internal/sandbox* 内部允许）；⑥禁止绕过 ToolRegistry 调用工具实现的具体 struct。 | M7→M4,M6,M13 |
| XR-11 | **文件系统操作分层规范**：L0 基础设施（`internal/`）可直接 `os.Open/WriteFile`（DB 初始化、schema 加载）；L1 执行沙箱（`internal/sandbox*`）可在 WorkDir 隔离边界内操作；L1/L2 认知·协同层（`internal/agent/`, `internal/swarm/`）禁止直接 `os.*` 文件操作，必须通过 WorkspaceManager 接口或 ToolRegistry 工具；L3 治理接口层（`internal/eval/`, `internal/gateway/`）禁止直接文件操作。 | M7→全模块 |
...
### [Sandbox-L2]
Rust 脚本沙箱（wasmtime_engine.rs via FFI，定义见 07-Tool-Action-Layer, §4.3）。deny-by-default，资源硬限制(RAM/CPU/Walltime)。TypeScript/Python 技能通过 npx tsx/python 在隔离环境中执行。
...
```
M7 物理归属 `internal/action/`（执行层：工具注册、沙箱）和 `internal/extension/`（技能、市场、MCP双向、原生扩展）。M7 的核心接口（Tool 类型、CapabilityLevel、ToolRegistry 接口）由 `internal/agent/` 中的 M4 通过 Go interface 消费——此为单向依赖 `cognition → action/extensions`，不构成循环引用。
...
### [Script-Sandbox]
Rust 脚本沙箱（wasmtime_engine.rs FFI，07-Tool-Action-Layer §4.3）。技能以 TypeScript/Python 脚本安装，通过 ContainerSandbox 执行（npx tsx / python）。M6 分层缓存: 热技能进程预热/冷技能按需启动。Tier 0 脚本并发默认 4。
...
- `[Spreading-Activation]`: 扩散激活图遍历。种子实体 energy=1.0，每轮 ×edge.weight 传播，自身 ×0.7 衰减（`energyDecay=0.7`，`MakeExtensionSearchFn` 硬编码），≤0.05 停止（dormancyThreshold=0.1），最多 5 轮（depth=2，fanOutLimit=5）。
- `[Connector-Taint-Table]`: Connector 源 TaintLevel 初始等级映射表（见 M11 §2.4）。ConnectorScheduler 在 Ingest 前按此表为每个外部数据源打标。
- `[Context-Engineering]`: 上下文工程 — 上下文窗口是有限预算，"放什么"比"怎么写"更重要。物理载体: `kernel.PromptBuilder` 三 Zone（ZoneImmutable/ZoneMutableSkill/ZoneTaintedData）+ ImmutableCore + Spotlighting（TaintedData 写入前强制包裹）。区别于 Prompt Engineering（关注单次提示措辞），Context Engineering 关注跨轮次/跨任务的上下文构造、压缩、分区、来源标注。参考 Anthropic *Effective context engineering*（2025）。
