...
- `[ThinkingMode]`: Polaris 内部推理深度决策枚举，`ThinkingDisabled` / `ThinkingHigh` / `ThinkingMax` 三档。由 `internal/observability/metrics/metrics_handler.go` 的 `SelectPlanTier(escalation, complexity, surpriseIndex)` 与规划模型池一并计算（ADR-0101 决策六：LLM 判难度、程序持策略，只有能力类失败升级；污点不参与），M4 `transitions.go` 在 LLM 调用前经 `protocol.WithThinkingMode` 注入 InferOption。**与 [ReasoningEffort] 是「决策层 → 抽象层」的映射关系，不是同义词**：ThinkingMode 是 Polaris 自己算出的"该想多深"，ReasoningEffort 是把它翻译成 Provider 能理解的字段。映射表见 M1 §5.2-bis。
