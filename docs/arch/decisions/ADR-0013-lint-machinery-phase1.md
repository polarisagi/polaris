...
`internal/` 包被任意层引用（不入 deny）；`internal/gateway` ↔ `internal/eval` 同 L3 互引允许（B1 未明禁）。
...
| 2026-07-03 | 补充记录：对于 `wrapcheck` 和 `gochecknoglobals` 的存量违规，采取了"渐进式路线"，在 `.golangci.yml` 中使用 `exclude-rules` 根据路径进行豁免。后续新增代码必须严格遵守规则，旧代码在重构或触碰时逐步清理（技术债）。 |
| 2026-07-04 | Phase 2 `funlen` 去留判断落地（任务24审计修复）：**不采用**。判断依据——`funlen` 检查物理行数/语句数，是复杂度的粗糙代理指标，与本 ADR 已启用的 `gocyclo`（min-complexity:15）高度冗余；Go 语言错误处理惯例（`if err != nil { return }` 密集出现）天然拉长物理行数但不代表真实复杂，`funlen` 在 Go 代码库里误报率偏高。前提"有其它规则顶替 funlen 复杂度治理职责"已满足（`gocyclo` 本仓库一直启用）。故复杂度治理职责收敛到 `gocyclo` 一家，`.golangci.yml` 不启用 `funlen`（见该文件行内注释）。`docs/specs/00-Constitution.md` §160 与 `docs/specs/06-Review.md` R7 checklist 中残留的 `funlen` 表述已同步移除，避免与本决议矛盾。 |
