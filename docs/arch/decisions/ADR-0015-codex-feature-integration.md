...
> **本节已被 [ADR-0016](./ADR-0016-unified-trust-extension-model.md) 取代。** Plugin Catalog 正确位置为 M13 `internal/gateway/server/plugin/catalog.go`，Plugin 安装 API 统一至 `/v1/plugins/`。Plugin 物理结构、Manifest 格式、TrustTier 体系以 ADR-0016 §2.3 为准。本节保留历史决策依据。
...
**决策**: 在 `internal/swarm/` 新增 `agent_profile.go` 支持从 `.polaris/agents/*.yaml` 加载自定义 Agent 配置，映射到现有 AgentCard。
...
**决策**: 废弃用户手动编写模板的假设，在 `internal/learning/` 下新增 `skill_creator.go` 机制。
