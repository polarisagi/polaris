...
- **日期**: 2026-05-16
- **决策者**: 架构组
- **相关模块**: M11 / `internal/security` / `rust/substrate`
...
- ABI 版本号双侧校验：major 不匹配 → panic（不可恢复，防 silent corruption）；minor 不匹配 → slog.Warn + 继续运行（建议重新编译 Rust dylib）
- ABI 变更视同 `B5.3` 跨语言破坏性变更,同步更新 `internal/protocol/ffi-abi.md`

同模式已延伸至 SurrealDB-Core FFI（`surreal_store.go`），cgo→purego 迁移已由 [ADR-0011](./ADR-0011-cgo-to-purego-migration.md)（2026-05-16）执行完毕。
