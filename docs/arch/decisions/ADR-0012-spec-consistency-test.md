...
| M4 状态转移 | `par.transitions` | `internal/agent/fsm/state_machine.go` Transitions |
| TaintLevel 五级 | `taint` | `internal/security/taint/taint.go` |
| KillSwitch 三阶段 | `kill_switch.stages` | `internal/security/killswitch.go` |
...
- 涉及对齐的 Go 文件：`internal/protocol/interfaces.go`（AgentState）/ `internal/agent/fsm/state_machine.go`（Transitions）/ `internal/security/taint/taint.go` / `internal/security/killswitch.go`
