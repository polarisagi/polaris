...
- `pkg/action/taint_preserving_decoder.go`（实现，保留在 pkg/action/）
- `pkg/extensions/mcp/mcp_client.go`（接入点；MCP Client 已迁移至 L2 扩展层）
...
| 2026-06-13 | 接入点路径更新：pkg/action/mcp_client.go → pkg/extensions/mcp/mcp_client.go |
