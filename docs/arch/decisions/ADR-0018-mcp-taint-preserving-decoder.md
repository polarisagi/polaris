...
- **反例守护**：未来如有人提议「MCP 客户端某个 ad-hoc 字段读取走 `json.Unmarshal` 走捷径」——拒绝。物理边界一旦破例即作废防线。建议 CI lint 检测 `internal/action/mcp_*` 文件中的 `encoding/json` import。
...
- `internal/extension/mcp/taint_decoder.go`（实现已迁移至 extension/mcp/；原 internal/action/ 文件以 `//go:build ignore` 标记排出编译）
- `internal/extension/mcp/mcp_client.go`（接入点；MCP Client 已迁移至 L2 扩展层）
...
- `internal/security/taint/taint.go`（TaintedString / TaintedJSONNode）
...
| 2026-06-13 | 接入点路径更新：internal/extension/mcp/mcp_manager.go → internal/extension/mcp/mcp_client.go |
| 2026-06-30 | 实现路径迁移：internal/action/taint_preserving_decoder.go → internal/extension/mcp/taint_decoder.go（归属层修正，重构 swarm/agents 错位清理顺带完成）|
