...
- **门控**：`internal/lint/ffi_abi_lint_test.go` `TestUintptrFFIArgHasKeepAlive`。

## 决策六：结构化载体禁字符串直拼

JSON 序列化、SQL FTS5 `MATCH` 查询以及 LLM Prompt 拼接三类结构化载体，禁止用 `fmt.Sprintf` 等直拼未转义的运行时动态数据。
- JSON 统一使用 `json.Marshal` 与具名结构体；
- FTS5 `MATCH` 参数统一过包内 `fts5QuoteLiteral` 转义；
- Prompt 非受信变量使用 `prompt.NewRandomBoundary` 包裹。

- **反例守护**：`episodic_mem_overflow.go` `"preview":%s` 直拼 JSON；`graph_traverser.go` 裸 `MATCH ?` 直拼实体名。
- **门控**：`internal/lint/security_lint_test.go` `TestNoRawStringIntoStructuredSink`。
...
> 注：`TestNoOrphanDocComment`（孤儿 doc comment）沿用 `docs/specs/00-Constitution.md §R7`，在 `doc_comment_name_test.go` 补齐 AST 自动化门控。
