...
**发现 2**: `internal/gateway/server/plugin/catalog.go` 证实 **M13 已有 Plugin Catalog（MCP 专属）**，这是 Plugin Registry 的正确位置。ADR-0015 把 Plugin Registry 放 M7 是临时错误决策。
...
预置受信 Publisher 列表（实现为硬编码配置，位于 `internal/config/trusted_publishers.go`），离线验证方案：
...
**验证策略**（双模式）：
- **离线（默认）**：Publisher ID 白名单 + content hash pinning（哈希机制如 `official-registry.yaml` 暂缓，当前依赖内置配置）
...
Plugin 的物理文件布局、Manifest 格式、ID 定义及其多组件结构已被纳入核心系统规范中，详见 [M13-bis-Extension-Registry](../M13-bis-Extension-Registry.md)。

### 2.4 Skill 格式最终确认

**agentskills.io SKILL.md 是唯一权威来源格式**（内外统一）：
目前所有关于 SKILL 目录结构（含 `.py`、`SIGNATURE`）及 Polaris 扩展字段（`agents/polaris.yaml`）等运行时配置均已移至 [M13-bis-Extension-Registry](../M13-bis-Extension-Registry.md) 进行统一维护。
...
| 2026-07-09 | 修正 §2.2 官方 Publisher 白名单的实现形式为 `internal/config/trusted_publishers.go` 硬编码，非 YAML 文件 |
