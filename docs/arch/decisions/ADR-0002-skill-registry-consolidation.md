...
- **日期**: 2026-05-16
- **决策者**: 架构组
- **相关模块**: M6 / `internal/extension/skill`

## 上下文

`internal/extension/skill/skill.go` 同时定义两套并行的 Registry 接口：
...
- `internal/extension/skill/skill.go`（重构目标）
- `internal/extension/skill/skill_test.go`（测试已对齐 protocol.SkillMeta，无需修改）
...
- `docs/specs/07-Reference-Implementation.md`（`internal/extension/skill` canonical 行依赖本 ADR）
