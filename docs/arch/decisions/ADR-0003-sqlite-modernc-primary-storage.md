...
> 2026-08-09 追记：重新评估触发条件——① 出现真实多用户/多进程并发写需求（当前
> 单用户单进程假设失效）时重议 Postgres/MySQL；② SurrealDB kv-mem 在 Tier-0
> 8GB 场景下的内存占用/性能实测持续超预算且 kv-rocksdb 也无法兜底时，重议是否
> 拆分认知检索轴到独立引擎；两者任一发生前，反例守护清单里的方案维持已驳回。
