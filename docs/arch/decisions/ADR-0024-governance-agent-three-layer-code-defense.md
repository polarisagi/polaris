...
- **日期**: 2026-06-13
- **决策者**: 架构组
- **相关模块**: M07 (Tool/Action), M11 (Policy/Safety), pkg/swarm/agents
...
- `pkg/swarm/agents/governance_agent.go`（`ValidateCodeWithAudit`，三层调用编排）
- `pkg/swarm/agents/code_validator.go`（Layer 0 AST + Layer 1 正则）
- `pkg/swarm/agents/security_audit_agent.go`（Layer 2 单次 ThinkingMax LLM）
- `pkg/swarm/agents/memory_agent.go`（`LLMInferFunc` 签名变更）
