...
- `internal/observability/probe/hardware_probe.go`（Tier 枚举 + `computeTier()` 按 RAM 判定 8/16/24/64GB）
- `internal/observability/probe/feature_gate.go`（`FeatureGate`/`featureRule`，MinTier + MinMemoryMB 双维度门控与降级序）
- `internal/observability/probe/tier_parameters.go`（`TierParameters`，各模块 Tier 相关数值参数）
- `internal/observability/probe/memory_probe.go`（OS 内存探测，供 Tier 判定使用）
