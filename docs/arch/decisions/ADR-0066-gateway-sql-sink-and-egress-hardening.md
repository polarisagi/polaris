# ADR-0066: Gateway 治理合集（SQL 下沉 + Egress 收紧 + Channel 适配器重构 + ChatOrchestrator 拆分路线，含原 ADR-0064/0067）

- **状态**: Accepted（决策一/二/三已执行）/ Proposed（决策四暂不实施）| **日期**: 2026-07-23（合并 2026-07-28）| **模块**: `internal/gateway/server/` / `internal/gateway/egress/` / `internal/channel/`

## 决策一：直连 SQL 下沉 Repository（A-4，原决策）
...
## 决策四：Gateway God Class 拆分路线（原 ADR-0067，Proposed，暂不实施）
...
分三步落地：① SQL 下沉（决策一，已完成）② 统一入站分发（决策三，已完成）③ 抽取 ChatOrchestrator 核心逻辑（**待排期，本轮不实施**，因 `sse.go` 是高频热路径，big-bang 重构风险过大）。

## 引用代码

`internal/protocol/repo/repo_automation.go`、`internal/gateway/egress/egress_gateway.go`、`internal/channel/adapter/adapter.go`、`internal/channel/manager.go`、`internal/gateway/server/chat/sse.go`
