...
1. `ParallelSample` 从未在 `pkg/substrate/inference/` 中实现，路由层仅支持单路推理
...
`SelectThinkingMode(SurpriseIndex, replanCount, TaintLevel)` 在 M4 `transitions.go` 中决定三档：
...
- `pkg/substrate/inference/adapter_deepseek.go`（thinking 字段写入 + ReasoningContent 提取）
- `pkg/substrate/inference/client.go`（`OpenAIRequest.Thinking` / `ReasoningEffort` 字段）
- `pkg/cognition/kernel/agent_execute.go`（`SelectThinkingMode` 调用 + `LastReasoningContent` 存入）
- `pkg/cognition/kernel/state_machine.go`（`StateContext.LastReasoningContent` 字段）
