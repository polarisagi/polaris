...
- **2026-09-21 追记**：本 ADR 定的是"校验值凭什么可信"，**被替换的文件放在哪里**另见 ADR-0096 决策四——桌面版的守护进程与 `lib/` 安装在 app bundle 之外（`~/.polarisagi/bin/`），故本模块的就地替换逻辑在桌面渠道与 tarball 渠道**行为完全一致**，无需为桌面版开分支。项目无平台签名证书，安装包完整性同样由本 ADR 的 cosign 锚点承接（ADR-0096 决策八）。
