...
- **正向**: DeepSeek V4 减少外部 API 依赖，默认 provider 统一降低配置分散风险
- **负向**: 依赖 DeepSeek 服务可用性；本地模型需 Tier1（16GB+）才能运行
- **反例守护**: 未来如有人提议将 OpenAI GPT-4 / Anthropic Claude 设为全系统默认 provider，引用本 ADR 拒绝；多 provider 共存是配置选项而非默认值

## 被驳回的方案

| 方案 | 驳回理由 |
|------|---------|
| 以 OpenAI GPT-4 为默认 | 成本不可控、无私有部署路径、与 provider-agnostic 目标冲突 |
| 不设默认 provider（全部用户配置） | 增加上手门槛，Tier-0 冷启动需要即开即用的默认值 |
