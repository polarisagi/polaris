...
> **§跳读**: 0-bis:5 职责 / 0-ter:15 不变量速查 / 1:26 技能表征 / 2:62 生命周期(CANONICAL) / 3:199 检索系统 / 4:228 演化 / 5:265 脚本缓存 / 7:297 340(SOFT)降级 / 8:311 依赖
## 0-bis. 职责边界
...
版本更新: 子技能 Version++ → SkillIndex 反向依赖扫描 → 标记父技能 `needs_compat_check` → 下次调用时半隔离脚本沙箱执行集成测试 (最新 10 个用例)。通过 → 自动更新 pin 版本；失败 → 保持旧版本 + WARN + 通知 M8。
