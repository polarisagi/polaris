...
> **§跳读**: 0-bis:5 职责 / 0-ter:15 不变量速查 / 1:26 技能表征 / 2:62 生命周期(CANONICAL) / 3:203 检索系统 / 4:232 演化 / 5:269 脚本缓存 / 7:303 340(SOFT)降级 / 8:317 依赖
## 0-bis. 职责边界
...
> **实现状态**：`FeatureLogicCollapse` 特性门控已定义（Tier 0+，≥8GB 且 ≥1GB free 自动启用）。蒸馏流水线：新鲜度检查 → 数据脱敏 → 编译门控 → LLM 代码生成（TypeScript）→ 静态 CFG 分析 → 沙箱行为测试 → 风险分级 → HMAC-SHA256 签名 → Skill 注册写入。M9 触发器位于 `internal/learning/`（在线语义方差估算 + HITL 分流）。<8GB VPS 仅加载预生成技能，Logic Collapse 被编译门控拦截。
...
当前 MVP 实现（`skill/compile.go:ValidateJS` + `skill/skill_pipeline.go:StaticAnalyzer`）：文本模式匹配 5 类禁止模式（`eval`/`new Function`/`require`/`os/exec`/`unsafe`）+ Go/TypeScript 导入审计。上述分析失败 → 拒绝生成，轨迹进入 MEMF + 写 `skill_static_analysis_rejected` 审计事件。

计划（Tier 1+）升级为完整 AST 分析，覆盖：
1. 控制流图 (CFG) 分析: 检测不可达代码、时间炸弹特征
2. 危险 API 审计: `child_process`/`fs`/`net` 等未声明能力
3. 确定性审查: `Date.now()`/`Math.random()`/`process.env` 等非确定性调用
...
- `internal/extension/skill/`：SkillRegistry（Register/List/Get）+ `compile.go` LogicCollapse 白盒分析蒸馏流水线（System 2 轨迹 → TypeScript 代码生成 → 静态 CFG 分析 → Wasm 注册）
