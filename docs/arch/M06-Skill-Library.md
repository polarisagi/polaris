...
> **§跳读**: 0-bis:5 职责 / 0-ter:15 不变量速查 / 1:26 技能表征 / 2:62 生命周期(CANONICAL) / 3:207 检索系统 / 4:236 演化 / 5:273 Wasm缓存 / 7:307 340(SOFT)降级 / 8:321 依赖
## 0-bis. 职责边界
...
对 `CompileRequest.TestCases` 中每个用例执行 wazero 沙箱验证（wasmExecutor 为 nil 时跳过）；Wasm 魔数验证（0x00 0x61 0x73 0x6D）在此阶段执行。
...
| M6 SkillSelector | `wasm` | `tool` | M4 System 1 命中 | SkillExecutor → wazero 沙箱 |
