...
> **§跳读**: 0-bis:5 职责 / 0-ter:15 不变量速查 / 1:26 技能表征 / 2:62 生命周期(CANONICAL) / 3:204 检索系统 / 4:233 演化 / 5:270 脚本缓存 / 7:304 340(SOFT)降级 / 8:318 依赖
## 0-bis. 职责边界

- M6 **是**: 技能注册、索引、检索、生命周期管理 | M6 **不是**: 技能沙箱执行（那是 M7）
- M6 **是**: Logic Collapse 蒸馏（System 2 轨迹 → Python 脚本生成 → ContainerSandbox 执行） | M6 **不是**: LLM 推理调用（那是 M1）
...
**Stage 5 — 脚本生成 + 索引 + 签名**: LLM 生成 Python 脚本（src/skill.py）经静态分析后 → ContainerSandbox 行为测试（Python 运行时已为 CodeAct 引入，零增量依赖）→ HMAC-SHA256 签名 → 写入 Skill Index → SurrealDB-Core KV 缓存。

### 2.2 Logic Collapse: System 2 → System 1

> **实现状态**：`FeatureLogicCollapse` 特性门控已定义（Tier 0+，≥8GB 且 ≥1GB free 自动启用）。蒸馏流水线：新鲜度检查 → 数据脱敏 → 编译门控 → LLM 代码生成（**Python**）→ 静态分析（ValidatePython）→ ContainerSandbox 行为测试 → 风险分级 → HMAC-SHA256 签名 → Skill 注册写入。M9 触发器位于 `internal/learning/`（在线语义方差估算 + HITL 分流）。<8GB VPS 仅加载预生成技能，Logic Collapse 被编译门控拦截。决策见 ADR-0026。

```
System 2 成功执行 → 轨迹分析 → LLM 代码生成 (Python: def execute(input: dict) -> dict:) → ValidatePython 静态检查 → ContainerSandbox 探针测试 → 签名入库
→ System 1: 同类任务 SurpriseIndex < 0.2 → Python 技能直接命中，零 LLM 推理 [SurpriseIndex]
```
...
当前实现：`skill/compile.go:ValidatePython`（文本模式匹配禁止 `import os` / `import subprocess` / `import socket` / `eval(` / `exec(`，允许 `import json` / `import re` / `import math` / `import datetime`）。分析失败 → 拒绝生成，轨迹进入 MEMF + 写 `skill_static_analysis_rejected` 审计事件。

计划（Tier 1+）升级为 Python AST 分析，覆盖：
...
解析轨迹数据，将字符串字面量/敏感标识符替换为参数化占位符，生成 `redaction_map.json` 存本地，确保 PII 不进入 LLM 生成路径。脚本通过 stdin/stdout JSON 传递参数。
...
> **实现状态**：当前 `SQLiteRegistryImpl.List` 在内存中二次过滤 capabilities，vecIndex/sigMatcher/depGraph 三路并行检索为设计规范（待实现）。**复核确认**：`internal/memory/retrieval/` 的 `HybridRetriever`（多路融合检索）仅服务于 M5 记忆检索（episodic/semantic），未被技能检索复用，两者是完全独立的两套实现，不存在"技能检索已借用记忆的统一检索接口"这种情况——本节仍是真实缺口，非命名/架构差异导致的误判。
...
- `internal/extension/skill/`：SkillRegistry（Register/List/Get）+ `compile.go` LogicCollapse 白盒分析蒸馏流水线（System 2 轨迹 → Python 代码生成 → ValidatePython 静态分析 → ContainerSandbox 行为测试 → DB 注册）
...
**双流水线区别**：`cognition/skill/compile.go` = 白盒轨迹分析驱动（LogicCollapse，自动化）；`extensions/skill/skill_creator.go` = 用户意图驱动 LLM 代码生成（手动触发，缺 exec_mode DB 写入路径，见下方审计缺口）。

> **审计缺口（P1，已知）：`exec_mode='ambient'` 注入路径**
> `internal/extension/skill/` 的 GenerateSkill 方法仅写文件系统（SKILL.md + plugin.json），不写 skills 表。LLM 自动生成的技能缺少 exec_mode 字段写入 DB 的路径。内置技能通过启动时 SeedBuiltinSkills 从 SKILL.md frontmatter 读取 exec_mode 并 UPSERT 到 DB，但 LLM 生成的技能不在 builtin 目录，无法通过此路径注册。修复方向：GenerateSkill 在写文件后调用 SkillRegistry.Register（含 exec_mode）写入 DB。
