...
> **部署说明**: TotalRAM ≥ 8GB 时系统自动将 `mem` 升级为 `rocksdb`（持久化落盘，~200MB RSS 开销）；<8GB VPS 维持 `mem`。也可显式配置 `backend = "rocksdb"` 强制开启。
...
- 引擎: **[Storage-SurrealDB-Core]** ([surrealdb](https://github.com/surrealdb/surrealdb) crate，Rust cdylib via purego, CGO-Free FFI)
  ...
  - > [!IMPORTANT]
    ...
    > - **surreal-rocksdb（自动或显式）**: `kv-rocksdb` 后端，TotalRAM ≥ 8GB 时自动启用（含 8GB Tier0 开发机）；RSS 开销 ~200MB，数据持久化落盘 `~/.polarisagi/polaris/data/surreal.db`；<8GB VPS 可显式设置 `backend = "rocksdb"` 强制开启。
