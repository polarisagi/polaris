...
> **§跳读**: 0-bis:5 职责 / 0-ter:18 不变量速查 / 1:31 黑板+CAS(核心) / 2:106 Supervisor / 3:123 编排模式 / 3-bis:141 SwarmRouter / 4:220 AgentCard / 5:234 Task分解 / 8:243 拓扑自演化 / 10:258 (SOFT)降级 / 11:277 跨模块契约 / 12:316 Custom Agent / 13:356 CSV Fan-out
## 0-bis. 职责边界
...
✅ **已实现**：`pkg/swarm/startup.go` 提供 `PhasedStartup` + `Pinger` 接口，按 P0→P4 顺序启动，每阶段 30s HealthCheckGate（errgroup 并行 Ping）；P0 失败调用 OnFail panic（策略真空不可接受），P1-P4 失败返回 error；`SQLiteBlackboard` 实现 `Ping(ctx)` 接入 P0 健康检查。
