...
| inv_M8_06 | 委托链深度 ≤3——跨 Agent 委托禁止超过 3 层 | M11 §8 Layer 4 多 Agent 宪法 | ⚠️ P1 缺陷：`TaskEntry.SpawnDepth` 字段存在（`internal/protocol/types.go`），但 `SQLiteBlackboard.PostTask/PostBatch` 从未校验，代理可无限递归派生（ROUND19 H2 修复中：加 MaxSpawnDepth=3 前置检查） |
...
> ⚠️ **代码偏差**：`SupervisorEpoch.Increment()` 当前使用非原子 `epoch++`，缺少 sync.Mutex/atomic 保护（并发不安全，P1 缺陷）。
...
> ⚠️ **代码偏差**：内存版 `SideEffectPreCheck` 未检查 `claimedVersion`（入参完全未使用），仅 SQLiteBlackboard 版本正确校验，违反 ABA 防护要求（P1 缺陷）。
...
> ⚠️ **代码偏差**：Reaper 引入了文档未描述的 toxicity 计数机制（`toxicity+1 >= 3` 直接标为 Failed），且宽限期内持锁逻辑存在与 CompleteTask 的竞态风险（P0 缺陷）。

**阶段 2（30s，内存守卫）**：清理 Status∈{Done,Failed} + UpdatedAt+5min<now 的终态任务，len>50000 时追加驱逐。释放 Lock 后逐条通过 MutationBus 归档，失败则写入 `~/.polarisagi/polaris/workspace/reaper_fallback/{taskID}.pb`。
...
- **动态提权防饥饿**（⚠️ **计划中，未实现**）：设计目标：Priority=2 待机 >5min 自动升至 1；Priority=3 等待 >15min 强制提权；Auto-Curriculum 升至 1 后再饥饿 10min → 保持 Priority=1 + WARN。当前 `dispatchPendingTasks` 仅按 `priority ASC` 排序，无任何计时提权逻辑。

无中央调度，优先级排序 + CAS 隐式竞争。

### 1.9 Phased Startup

> ⚠️ **未实现**：Phased Startup 在代码中不存在。`NewOrchestrator` 直接初始化，无 P0→P4 分阶段启动逻辑，无 HealthCheckGate 30s 超时保护，无策略真空窗口防护。

**设计目标**（供实现参考）：P0 先启动 Policy/Cedar-Gate/Blackboard/SQLite，之后 P1 Memory/Knowledge，P2 Skill/MCP，P3 Orchestrator/Planner，P4 Worker/Reviewer。每层有 30s HealthCheckGate；M11 Cedar-Gate 在 P0 即启用 deny-by-default，P0 完成前阻断所有工具注册与任务执行。
...
| 缺口 | 严重度 | 说明 |
|------|--------|------|
| inv_M8_02：Blackboard → EventLog 双写 | ⚠️ P0 | SQLiteBlackboard 直写 tasks 表，未写 events 表；崩溃后无法 EventLog 回放重建 |
| 动态提权防饥饿（§1.8） | ⚠️ P1 | 未实现；当前仅按 priority ASC 排序 |
| Phased Startup P0→P4（§1.9） | ⚠️ P1 | 未实现；NewOrchestrator 直接初始化 |
| 委托链深度 ≤3 校验（inv_M8_06） | ⚠️ P1 | `PostTask/PostBatch` 无 SpawnDepth 校验，代理可无限递归派生（ROUND19 H2 修复中） |
| 内存版 SideEffectPreCheck Version 校验 | ⚠️ P1 | claimedVersion 入参未使用；仅 SQLiteBlackboard 版本正确 |
| SupervisorEpoch 原子性 | ⚠️ P1 | epoch++ 非原子操作，并发不安全 |
| 崩溃恢复并发 cancel（§1.7） | ⚠️ P1 | 文档要求 errgroup 并发，代码当前串行 |
| CompleteTask 阻塞写 channel | ⚠️ P0 | 内存版 CompleteTask 阻塞式写 events chan（cap=1024），高并发下可能死锁 |
