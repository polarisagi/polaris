...
**阶段 2（Phase 2 GC，✅ 已实现）**：`reaperPhase2` 每秒随 Phase 1 一同触发，清理 `running` 状态超 5 分钟的僵尸任务（先 cancel goroutine 再标 failed）和 `pending` 状态超 5 分钟的饥饿任务（直接标 failed），防止 SQLite 黑板在高负载下体积失控。
