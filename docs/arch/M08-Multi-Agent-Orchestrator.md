...
**独立子包**：`internal/swarm/agents/`，与本节 §2 Supervisor Tree、§3 编排模式均**正交**——包内 Agent 均为常驻 goroutine，通过 channel 与主脑通信，**不经过 SQLiteBlackboard 的 tasks 表**，Tier-0 约束下两 Agent 静止时内存开销 < 5MB。
