...
> **§跳读**: 0-bis:5 职责 / 0-ter:18 不变量速查 / 1:31 黑板+CAS(核心) / 2:106 Supervisor / 2-bis:125 常驻角色Agent / 3:140 编排模式 / 3-bis:158 SwarmRouter / 3-ter:196 PipelineOrchestrator / 4:237 AgentCard / 5:251 Task分解 / 8:260 拓扑自演化 / 10:275 (SOFT)降级 / 11:294 跨模块契约 / 11.2:318 已知实现缺口 / 12:332 Custom Agent / 13:370 CSV Fan-out
## 0-bis. 职责边界
...
产品侧设想的 Librarian（对接用户本地知识库如 Obsidian/Notion，整理内容、辅助 AI 理解用户/办公）**不新建独立 swarm 角色**——2026-07-03 系统级决策：这类"周期性把外部信息摄入进 Agent 可用知识"的职能与 MemoryAgent 已经承担的 Extension Librarian 调度是同一性质的工作，收编进 **MemoryAgent** 的调度范围，不单独起一个常驻 goroutine（避免 Tier-0 内存预算和 Supervisor 挂载点的边际浪费，两者也没有独立扩缩容/故障隔离的必要性）。`internal/knowledge/connector/`：`obsidian_connector.go` + `sync_scheduler.go` 已实现 Obsidian 同步，改为由 MemoryAgent 触发调度而非独立运行；**Notion 连接器待实现**。若未来知识库同步的复杂度/调用量确实超出 MemoryAgent 承载范围，再考虑拆分独立角色。详见开发提示词 `local_playground/upgrade/2026-07-03-architecture-gap-remediation.md` P2-4。
...
| 个人知识库连接器调度 | ⚠️ 待开发 | 详见 §2-bis；决策为收编进 MemoryAgent（非新建角色），`internal/knowledge/connector/` 已有 Obsidian 同步待接入 MemoryAgent 调度，Notion 连接器缺失 |
