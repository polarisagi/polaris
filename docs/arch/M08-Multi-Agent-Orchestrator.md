...
<!-- §跳读: 0-bis:5 职责 / 0-ter:18 不变量速查 / 1:31 黑板+CAS(核心) / 2:106 Supervisor / 2-bis:125 常驻角色Agent / 3:142 编排模式 / 3-bis:172 SwarmRouter / 3-ter:210 PipelineOrchestrator / 4:304 AgentCard / 5:318 Task分解 / 8:327 拓扑自演化 / 10:342 (SOFT)降级 / 11:361 跨模块契约 / 11.2:318 已知实现缺口 / 12:401 Custom Agent / 13:439 CSV Fan-out -->
## 0-bis. 职责边界
...
> 实现: `internal/swarm/orchestrator/pattern_dag.go`
...
> 实现: `internal/swarm/orchestrator/pattern_state_graph.go`（`StateGraphExecutor`）+ `pkg/graph/state_graph.go`（`ValidateStateGraphTopology`）
