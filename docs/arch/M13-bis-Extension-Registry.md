...
> **§跳读**: 0:8 职责边界 / 1:22 能力分层 / 2:52 扩展类型 / 3:90 技能执行模式 / 4:115 工具懒加载 / 5:147 安装流 / 6:318 信任门控 / 7:364 文件系统 / 8:394 调用路由 / 9:457 自动化 / 10:557 跨代理协作 / 11:592 学习技能归并 / 12:608 表引用
...
Layer 1  Instances（安装层）← SSoT
  extension_instances   所有已安装扩展的统一记录
                        字段：id / ext_type / origin / catalog_id / name / publisher /
                              trust_tier / runtime_id / install_path / config / status / error_msg
                        注：enabled（废弃）、parent_id（废弃）已删除

Layer 2  Runtime（运行时层）
  mcp_servers（015）    所有 MCP 进程配置；含独立 MCP（plugin_id=''）和插件子 MCP（plugin_id!=''）
                        新增字段：plugin_id（FK→plugins.id）/ work_dir（插件根目录）
  skills（008）         script/wasm 执行元数据；独立 Skill（plugin_id=''）和插件子 Skill（plugin_id!=''）
                        新增字段：plugin_id（FK→plugins.id）/ name 格式统一为 "skill:{slug}"
  plugins（021）        插件运行时状态（install_path + enabled + mcp_policy + manifest）
                        mcp_policy 仅存 approval_mode/enabled_tools 等附加策略；enabled 状态以 mcp_servers.enabled 为权威
  apps（028）           富交互应用（Codex App）；runtime_id 指向此表，替代原 config JSON 存储
...
|-----------|------|---------|---------|
| `tool`（默认） | script runtime：暴露为 `skill__{slug}` LLM 工具（双下划线前缀）；wasm runtime：经 `execute_skill` MCP 工具调用（M7 §1） | 按需，LLM 决策 | 专项任务技能（代码审查、PR 规范） |
...
- wasm 技能由 Polaris MCP Server 的 `execute_skill` 工具代理，不直接注入 LLM 工具列表
- 两条路径互不干扰
...
```
正常模式（tools ≤ LazyLoadThreshold）：
  buildToolSchemas() 全量返回所有 builtin + mcp + skill(exec_mode=tool,runtime=script) 的 schema

懒加载模式（tools > LazyLoadThreshold）：
...
POST /v1/plugins/install {catalog_id, type=skill}
  ...
  5. SkillRegistry.Register(SkillMeta{name="skill:{hex}", runtime="script", plugin_id="", ...})
     → INSERT INTO skills（经 SkillRegistry 校验，name 须含 "skill:" 前缀）
  6. UPDATE extension_instances SET status=installed, runtime_id="skill:{hex}"

  exec_mode='tool':    buildToolSchemas() 以 "skill__{slug}" 工具名注入 LLM
  exec_mode='ambient': injectSystemPrompt() 从 skills 表 SELECT WHERE exec_mode='ambient' 注入上下文
```
...
POST /v1/plugins/install {catalog_id, type=plugin}
  ...
  4. 解析 plugin.json (PluginBundleManifest)：
     mcp_inline / .mcp.json / 第三方格式 → 收集所有子 MCP 定义（name/command/args/env/url）
     skills/ 目录 / skills[] 数组 → 收集所有子 Skill（SKILL.md）
     hooks{} → 执行 install hook（路径防穿越校验）
  5. INSERT plugins(021)：install_path, enabled, mcp_policy（附加策略），manifest 快照
  6. 子 MCP → INSERT mcp_servers（id="plugin_{pl_id}_{name}", plugin_id, work_dir=install_path）
              → MCPManager.Add() 异步连接
  7. 子 Skill → SkillRegistry.Register(SkillMeta{name="skill:{plugin-name}/{slug}", plugin_id})
               → INSERT skills（exec_mode='ambient'→上下文注入；'tool'→LLM 工具）
  8. UPDATE extension_instances SET status=installed, runtime_id="pl_{hex}"

运行时 MCP 加载：
  启动时: MCPManager.LoadFromDB(WHERE enabled=1) → 统一加载（含 plugin_id != '' 的子 MCP）
  MCPClientConfig.WorkDir = mcp_servers.work_dir（= install_path，保证 uv/npm 正确解析依赖）

Skill 注入：
  buildToolSchemas(): SELECT FROM skills WHERE exec_mode='tool' AND deprecated=0
  buildAmbientSkillsSection(): SELECT FROM skills WHERE exec_mode='ambient' AND deprecated=0
```

**插件生命周期级联**：

```
禁用插件 (PUT /v1/plugins/{id} {enabled:false}):
  UPDATE mcp_servers SET enabled=0 WHERE plugin_id=?
  UPDATE skills SET deprecated=1 WHERE plugin_id=?
  MCPManager.Remove(plugin_{id}_{name}) × N

启用插件 (PUT /v1/plugins/{id} {enabled:true}):
  UPDATE mcp_servers SET enabled=1 WHERE plugin_id=?
  UPDATE skills SET deprecated=0 WHERE plugin_id=?
  startMCPServer() × N（从 mcp_servers 读配置重启）

切换子 MCP (PATCH /v1/plugins/{id}/mcp/{name} {enabled:bool}):
  UPDATE mcp_servers SET enabled=? WHERE id=? AND plugin_id=?
  MCPManager.Remove() 或 startMCPServer()
  （不再操作 mcp_policy.enabled，mcp_servers.enabled 是唯一权威）

卸载插件 (DELETE /v1/plugins/{catalogID}):
  MCPManager.Remove() × N
  DELETE FROM mcp_servers WHERE plugin_id=?     ← 硬删除
  DELETE FROM skills WHERE plugin_id=?           ← 硬删除
  DELETE FROM plugins WHERE id=?
  os.RemoveAll(install_path)
  DELETE FROM extension_instances WHERE catalog_id=?
```
...
DELETE /v1/plugins/{catalogID}
  1. 查 extension_instances WHERE catalog_id=?（单条记录；parent_id 机制已废弃）
  2. 按 ext_type 清理运行时：
     mcp        → MCPManager.Remove(runtimeID) + DELETE mcp_servers WHERE id=runtimeID
     skill      → DELETE skills WHERE name=runtimeID          ← 硬删除
     plugin     → MCPManager.Remove(plugin_{id}_*) × N
                  DELETE mcp_servers WHERE plugin_id=runtimeID ← 硬删除
                  DELETE skills WHERE plugin_id=runtimeID      ← 硬删除
                  DELETE plugins WHERE id=runtimeID
     app        → DELETE apps WHERE id=runtimeID
     automation → Scheduler.Deregister() + DELETE automations WHERE id=runtimeID
     agent      → MCPManager.Remove(runtimeID) + DELETE mcp_servers WHERE id=runtimeID
  3. os.RemoveAll(install_path)（内部经 safeJoin 路径校验）
  4. DELETE extension_instances WHERE id=?
  5. 非 builtin 来源 → cleanCatalog() 级联 DELETE extension_catalog
```
...
func buildToolSchemas() []ToolSchema {
  if totalTools() <= LazyLoadThreshold {
    // 正常模式：全量（builtin + mcp + script runtime skill[exec_mode=tool]）
...
> `skillToolSchemas()` 仅暴露 `runtime='script' AND exec_mode='tool'` 的技能，工具名格式为 `skill__{slug}`（DB 存储键 `skill:{slug}` 去掉前缀后加双下划线）。wasm runtime 技能经 Polaris MCP Server `execute_skill` 工具代理，不进入此列表。
...
LLM tool_use {name, input}
  ...
  ├── "skill__{slug}"  → DB 读 skills.instructions (WHERE name='skill:{slug}')
  │                       + input → 拼接返回（script runtime，LLM 执行 instructions）
  ├── "execute_skill"  → MCPManager → Polaris MCP Server
  │                       → skillLib.LookupByName → WasmSandbox.Execute（wasm runtime）
  ├── "agent:{id}"     → Cedar Gate 检查 → A2A Client → 外部 Agent 端点
  │                       → 结果赋 TaintHigh（M7 §2）
