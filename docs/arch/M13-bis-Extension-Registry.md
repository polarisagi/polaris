...
> **§跳读**: 0:8 职责边界 / 1:22 能力分层 / 2:41 扩展类型 / 3:79 技能执行模式 / 4:104 工具懒加载 / 5:133 安装流 / 6:207 信任门控 / 7:254 文件系统 / 8:285 调用路由 / 9:320 自动化 / 10:392 跨代理协作 / 11:418 学习技能归并 / 12:445 表引用
...
✅ **已实现**：`internal/tool/catalog/composite.go` 支持基于 `session_id` 的激活状态跟踪。`BuildToolSchemas()` 根据阈值动态过滤未激活的非核心工具。
...
**MCP 客户端三种传输**（`internal/extension/mcp/mcp_client.go`）：`stdio`（子进程 JSON-RPC）、`sse`（HTTP Server-Sent Events）、`http`（ADR-0017 Streamable HTTP）。`sanitizeParentEnv()`（`internal/extension/mcp/env.go`）仅向子进程传递白名单无害系统变量，防环境变量注入。
...
> ⚠️ **代码偏差**：Automation 安装流经由 `Manager.InstallExtension()` 的路径同 §5.3 描述的空壳问题。cron/webhook/manual 触发已实现；**event 触发 ✅ 已实现**（`eventTick` 轮询 `events` 表，按 `event_filter` JSON 匹配 topic/type，复用 `cronTick` 周期执行，`017_automations.sql` 新增 `event_filter TEXT` 列）；专属 `github` trigger_type 枚举仍待开发，**但 ✅ GitHub 事件触发能力本身已实现**（走通用 webhook 基础设施，非空白）——`internal/gateway/server/sysadmin/channels.go` 的 `HandleWebhookReceive` 是协议无关通用 webhook 接收器（`POST /v1/webhooks/{channelType}/{channelID}`），`verifyWebhookSource` 已对未知渠道类型走 `X-Hub-Signature-256` HMAC-SHA256 验签（即 GitHub 标准签名协议），配合已实现的 `webhook` trigger_type 即可接收并触发 GitHub 仓库事件。真正缺的是"github"专属枚举值和对 PR/issue 等事件语义的结构化解析（当前只能拿到原始 payload，无法按 `pull_request.opened` 这类事件类型细分路由）。
...
DAG 执行器复用 M4 DAGExecutor（`internal/agent/`）。Workflow 数据由独立的 `workflows` + `workflow_steps` 主从表持久化（见 `internal/protocol/schema/029_workflows.sql`），automations 表通过 `workflow_id` 外键关联，不使用内联 JSON 字段。
