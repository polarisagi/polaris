...
| inv_M1_06 | API Key 经 `credentialFn() []byte` JIT 获取，通过 `keyInjectRT` transport 在 RoundTrip 期间注入 Header，RoundTrip 返回后立即 `Header.Del` + `clearBytes` 清零原始切片 | code review |
...
- API Key 通过 `credentialFn() []byte` JIT 获取；`keyInjectRT` transport 在 HTTP RoundTrip 期间将 key 注入 Header，RoundTrip 返回后立即 `Header.Del("x-api-key")` + `clearBytes(apiKey)` 清零原始切片。注：`string(apiKey)` 转换产生的 Go 不可变字符串副本在 GC 前短暂存在于 heap，见 `docs/upgrade/fix-api-key-memclr.md`
