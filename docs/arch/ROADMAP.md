...
13 模块架构选型与后端代码实施已于 **2026-05-20 全部闭环**（276 个 Go 源文件，202 项自动化测试通过）。`internal/` 完全对齐 `spec/state.yaml` + DDL + 本架构体系，物理隔离、Taint Tracking、SurpriseIndex、Edge Streaming 等关键能力均已落地。
...
- **L1 cognition/action**: Agent FSM（10 态）+ DAG Executor + PromptBuilder + 四层记忆 + Consolidation；Rust 脚本沙箱 + MCP Client/Manager + CapabilityToken + CodeAct + LAM + Computer Use（Darwin/Linux/Windows）+ Skill Pipeline
...
当前 `internal/learning/` 实现 Layer A/C 组合（embedding cosine + toolSeq + MEMF）。Layer B 为马尔可夫转移矩阵条件概率，定位为全 Provider 可用的控制流主信号。
