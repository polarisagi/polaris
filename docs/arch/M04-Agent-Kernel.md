...
> **§跳读**: 0-bis:5 职责 / 0-ter:18 不变量速查 / 1:31 状态机 / 2:86 Suspend-on-Idle / 3:98 S_VALIDATE / 4:134 DAG / 5:201 System1/2 / 6:225 WorldModel / 7:236 推理预算 / 8:291 CrashRecovery / 12:336 (SOFT)降级 / 13:362 跨模块契约
## 0-bis. 职责边界
...
| Context 隔离 | 共享父 Agent PromptBuilder | **每个子 Agent 持有独立 PromptBuilder 实例与 context window**，仅通过 Blackboard 结构化 result entry 交换（[Sub-agent-Isolation]） |
...
**SurpriseIndex 计算来源（ADR-0028 BUG-D）**：`populateSessionContext` 优先从注入的 `SurpriseReader`（`learning/surprise.SurpriseCalculator`，三分量：Jaccard + MEMF + Markov）读取；未注入时退回 `ComputeBasic`（仅 Jaccard）。计算结果同步写入 `metrics.GlobalSurpriseIndex`，供 `SelectThinkingMode` 消费。`SurpriseReader` 为 consumer-side 接口，防 L1→L2 包循环。
...
WorldModel 实现见 `internal/memory/`；上下文组装由 `internal/agent/` 的 `PromptBuilder` 统一执行（含三 Zone 布局 + TaintData Spotlighting 门控）。
...
> **BudgetManager**（`budget.go`）：独立预留模块，**当前未被 Agent 主路径使用**。Agent 使用 `StateContext.TokenBudget/TokensUsed` 内联计算，三级熔断（50%/75%/100%）见 §7.0。BudgetManager 供未来跨组件预算协调使用。
...
满足 [HE-Rule-6]（State-in-DB）——崩溃恢复从 M2 EventLog 回放，不依赖独立的 FSM checkpoint 文件。状态持久化时机：`runFSM` 在每次状态转移完成后显式触发写入（见 `internal/agent/fsm/transitions.go`），Transition.Effects 回调本身不负责持久化。SurrealDB-Core KV 的 goroutine checkpoint（§8.1）仅用于空闲时释放 goroutine 栈以节省内存，非崩溃恢复路径。
...
**`ErrAllProvidersFailed` 专项处理（全 Provider 熔断）**:
  ...
  **Provider 恢复唤醒**: M1 CircuitBreaker Open→Closed (§7.3) → M2 Outbox 投递 `target_engine:"m4_provider_recovery"` 事件。Handler 注册于 M2 全局 Outbox Worker（`internal/store/`），实现位于 `internal/agent/`——不在 M4 内独立 Worker（违反 M2 §2.3 单写者）。执行序列:
...
| M4→M5 | PromptBuilder / HybridRetriever | 记忆检索 + 上下文组装（PromptBuilder 三 Zone 布局）。M5 §2, §7 |
