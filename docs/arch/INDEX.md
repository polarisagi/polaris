...
- **排查起点**：顺着调用链找构造点，检查是否有零值结构体/丢弃的 `_ =` 返回值；历史真实案例见 `ADR-0027（Architecture Decision Record，架构决策记录）` BUG-1/BUG-2。
...
- **排查起点**：检查该状态机驱动 goroutine 是否用 `concurrent.SafeGo` 包装；历史真实案例见 `ADR-0027` BUG-3 / `ADR-0028` BUG-B。
