...
1. **大模型是协处理器**：Go 确定性状态机持有控制流；LLM 做概率性填空，不做自由编排。三角色 Swarm（Supervisor / Librarian / Governance）物理隔离，异步黑板协调。
2. **物理安全边界**：五级 Taint 只升不降 + Cedar deny-by-default + 三级沙箱；概率过滤不是安全边界（[HE-Rule-2]）。
3. **Eval 门控进化**：Prompt / Skill / Config / 源码补丁全部走 Staging 七阶段 CI；MEMF 持久化失败记忆，幻觉改进无法晋升至生产记忆。

---

## 0. SSoT 四层

L1 架构散文(本文档+模块文档) → L2 结构真相(`internal/protocol/schema/*.sql` DDL) → L3 状态机规约(`spec/state.yaml`) → L4 行为真相(`internal/` + `internal/protocol/*.go` 编译器强制)。
...
- 多 Agent 黑板 + CAS 协调,禁止自由 NL 对话；三角色默认拓扑（Supervisor + Librarian + Governance）
...
- 语音: STT 强制 Sherpa-ONNX + SenseVoice（零 Python 依赖，极低内存开销，Tier 0）；TTS 走 `tts.Provider` 三路配置（`edge` 默认 / `http` sidecar / `sherpa` 离线），见 ADR-0031。
...
> **与 M09 Rollout 实现映射**：本节 Stage 1-7 是全局流水线设计语言；`self_improve/rollout.go` 实现六阶段渐进发布（0→1%→5%→25%→100%→Committed），是 Stage 6 canary_rollout 的具体实现细化（详见 M09 §2.3）。两套编号体系不冲突——Stage 6 展开为 rollout.go 的六阶段门控，Stage 7 对应 Committed 状态落库。
...
| M9 Self-Improvement Engine | 完整 | ✅ 完整 | L2 LogicCollapse 编译后提交 StagingPipeline（SubmitCandidate）已实现；L3/L4 HITL 占位已写 |
| M10 Knowledge RAG | 完整 | ✅ 完整 | — |
| M11 Policy / Safety | 完整 | ✅ 完整 | Cedar FFI 降级时有 slog.WarnContext 可观测（无独立 Prometheus counter） |
