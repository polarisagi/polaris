...
**write_file 禁止目录黑名单**：`makeWriteFileFn`（`pkg/action/tool/builtin_tools.go`）在 `checkAllowedPath` 通过后追加 `checkForbiddenPath` 二次过滤，硬编码禁止写入 `~/.polarisagi/polaris/config/`、`~/.polarisagi/polaris/data/`、`~/.ssh/`、`~/.gnupg/`、`/etc/`、`/usr/`、`/bin/`、`/sbin/` 等敏感目录；路径比较走 `filepath.Clean + strings.HasPrefix`，不可被配置覆盖。
...
`pkg/action/tool/native_sandbox.go` — 为内置 `bash` 和 `run_command` 工具提供进程级隔离，与 Wasmtime Wasm 沙箱互补（Wasmtime 管 Wasm 技能，原生沙箱管系统进程）。
...
Wasm 沙箱由 Rust Wasmtime 引擎驱动，通过 purego FFI 桥接至 Go 层。实现文件：`pkg/action/tool/wasmtime_sandbox.go`（Go 封装）+ `pkg/action/tool/rust_wasmtime_sandbox.go`（purego FFI 桥）。
