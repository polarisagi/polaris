...
> **§跳读**: 0-bis:6 职责 / 0-ter:18 不变量速查 / 1:29 MCP双向 / 2:83 A2A / 3:111 注册 / 4:164 三级沙箱(CANONICAL) / 5:299 PolicyGate / 6:354 Capability / 7:371 动作扩展 / 8:506 Usage演化 / 12:547 (SOFT)降级 / 13:565 跨模块契约 / 14:585 Plugin / 15:627 Hook
## 0-bis. 职责边界
...
16KB 代码大小限制已实现（配置为 `MaxCodeSizeBytes`，默认 16384，`internal/action/codeact/code_act.go` 构造时通过 `WithMaxCodeSize` 传入强制阻断）。PostExec PII Redact 仍属文档设计目标，**[计划中，确认未开发]**。Taint 检查已在 `Execute` 流程中通过 Cedar 策略评估 + `sandbox.Level()≥3` 断言间接覆盖污点等级门控（见上一段"已实现并接线"），不属于本条遗留缺口。
