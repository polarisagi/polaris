...
WorkingMemory/ImmutableCore/ContextWindow/ScratchPad 接口定义见 `internal/protocol/interfaces.go`，实现见 `internal/memory/`。NotesStore 和 UserProfile 实现均位于 `internal/memory/` 和 `internal/swarm/agents/`。
...
实现见 `internal/swarm/agents/`（PersonaRefiner + UserProfile）。持久化到 `preferences` 表（单条 JSON）。
...
- 关联模块: M4 Agent Kernel | 关键契约: PromptBuilder（三 Zone 组装 + Spotlighting 门控）、HybridRetriever 上下文检索 | 位置: M4 §2, §10
...
- 关联模块: M10 Knowledge RAG | 关键契约: HybridRetriever 共享引擎（`internal/store/`）、检索配置差异 | 位置: M10 §2.2
