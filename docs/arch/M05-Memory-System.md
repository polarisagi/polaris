...
- **Stage 1 — 实体/关系提取**: 
  - 聚合 Session 内 Episodic 事件文本，优先调用 Provider LLM 提取命名的限定实体（`user_preference`, `constraint`, `temporary_conclusion`, `entity`）与特定关系（`depends_on`, `configures`, `conflicts_with`, `relates_to`），结构化输出 JSON。
  - LLM 不可用时降级为正则模式匹配。
- **Stage 2 — Upsert Semantic + Entity 生命周期**: 
  - 批量调用 `SemanticMemory.UpsertFact / UpsertRelation`。
  - 精确名称冲突 → `MarkEntitySuperseded(oldDBID)` 打标 `superseded` 后 INSERT 新版本。
  - `user_preference` 类型还额外执行 Jaccard 近重复检测（阈值 0.6），Jaccard 命中的旧实体同样打标 `superseded`。单条失败不中止批次。
- **Stage 3 — 会话摘要**: 
