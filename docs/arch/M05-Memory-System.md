...
> **§跳读**: 0-bis:7 职责 / 0-ter:19 不变量速查 / 1:30 四层映射 / 2:39 L0 Working / 3:116 L1 Episodic / 4:214 L2 Semantic / 5:254 L3 Procedural / 6:264 写路径 / 7:276 HybridRetriever / 8:353 EffConn / 9:363 Consolidation / 10:388 Forgetting / 11:405 ContextAssembler / 12:475 Drift / 14:513 496(SOFT)降级 / 15:535 依赖
## 0-bis. 职责边界
...
**kernel.PromptBuilder.Build**: 固定顺序 ZoneImmutable → ZoneMutableSkill → ZoneTaintedData。ZoneTaintedData 追加前（WriteUserData），对 `[TaintLevel] >= TaintMedium` 的内容执行 M11 Spotlighting 包裹：
...
> `episodic_events` 是 M2 `events` 表（[EventLog] 真相源）的派生投影表。**热路径**：`EpisodicMem.Append()` 将事件写入 `kv_store` KV 表，Payload 上界 8KB，超限落盘至 `~/.polarisagi/polaris/logs/events/<id>.bin` 并替换为 `log_ref` 占位符。**冷投影**：`agent_execute.go` 同步向 outbox 写入 `target_engine="episodic"` 记录，OutboxWorker `EpisodicProjectorHandler` 异步消费并 INSERT `episodic_events` 表，填充 `content`/`salience`/`decay_weight`/`cold` 等字段。
...
`HybridRetrieverImpl` 持有 store / graph / durative / reflectionMem / embedder 五个可选路径字段，提供四个构造器重载，按激活路径数量递增（从基础 BM25+Simhash 到含 Graph+DurativeMemory+ReflectionMem 的全路径），详见 `pkg/cognition/memory/retriever.go`。向量检索路径（`InjectEmbedder`）可在构造后注入。

5路融合检索（BM25+Simhash+Graph+ReflectionMem+DurativeMemory），RRF（k=60）融合排序，权重分别为 1.0/0.8/0.6/0.15/0.3。
...
**超时保护（已修复）**: `Run` 在调用方未设置 deadline 时自动注入 5 分钟超时（`consolidationTimeout`），防止 LLM 长调用阻塞 M9 调度器。见 `pkg/cognition/consolidation.go`。

> **审计缺口（P2）**: EpisodicMem.Query 冷启动路径在 `mu.Lock()` 内执行 `store.Scan`（KV 扫描），持锁期间存在 IO 阻塞风险。热路径（`len(em.events) > 0`）不受影响。修复方向：锁外执行扫描，锁内仅合并结果到 `em.events`。
...
PromptBuilder 布局实现见 `pkg/cognition/kernel/prompt.go`，SessionCompressor 实现见 `pkg/cognition/compressor.go`。
...
- 故障场景: Context 组装超时 (>500ms) | 降级路径: 跳过 ZoneMutableSkill 组装，仅发 ZoneImmutable + 摘要 | 恢复策略: 下次 context refresh 完整组装
