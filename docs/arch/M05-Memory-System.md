...
- Stage 2 — **Upsert Semantic**: 批量调用 `SemanticMemory.UpsertFact / UpsertRelation`。提取时引入冲突检测：若新旧实体同名且存在矛盾，以新实体优先，同时为旧实体添加 `"supersedes": existing.ID` 的取代标记并触发版本递增。新实体直接 INSERT。单条失败不中止批次。
