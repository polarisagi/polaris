...
pub extern "C" fn wasmtime_pool_init(_n: c_int) -> c_int {
    let result = panic::catch_unwind(|| -> c_int {
        GLOBAL_ENGINE.get_or_init(|| match EngineState::new() {
...
pub extern "C" fn wasmtime_init(out_err: *mut *mut c_char) -> c_int {
    let result = panic::catch_unwind(|| -> c_int {
        // Future enhancement: properly handle error if EngineState::new fails here
        // Currently we panic inside get_or_init on failure to match previous logic's simplicity,
        // but we could try to initialize it first and handle error gracefully.
        match EngineState::new() {
            Ok(state) => {
                let _ = GLOBAL_ENGINE.get_or_init(|| Arc::new(state));
                write_err(out_err, "");
                WASMTIME_OK
            }
            Err(e) => {
                let is_init = GLOBAL_ENGINE.get().is_some();
                if is_init {
                    write_err(out_err, "");
                    WASMTIME_OK
                } else {
