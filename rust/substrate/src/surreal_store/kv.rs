...
pub unsafe extern "C" fn surreal_kv_get(
    ...
    let result = panic::catch_unwind(|| {
        ...
        match res {
            ...
            Some(row) => {
                // 存储值解码失败 = 后端数据损坏，报 QUERY 错误，不返回半损坏 buffer。
                let val_bytes = match hex_to_bytes(&row.v) {
                    Some(b) => b,
                    None => {
                        eprintln!(
                            "[surreal_kv_get] corrupt hex value in kv row, len={}",
                            row.v.len()
                        );
                        return SURREAL_ERR_QUERY;
                    }
                };
                // 出参指针为空则无处回写：提前返回（val_bytes 正常析构，不泄漏）。
                if out_val.is_null() || out_len.is_null() {
                    return SURREAL_ERR_QUERY;
                }
