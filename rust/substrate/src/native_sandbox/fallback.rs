...
pub(super) fn exec_namespace_fallback(
    ...
    if let Some(unshare) = which_tool("unshare") {
        ...
        return run_with_timeout(
            ...
            timeout_ms,
            "namespace",
            req.max_memory_mb.unwrap_or(0),
        );
    }

    run_with_timeout(cmd, timeout_ms, "bare", req.max_memory_mb.unwrap_or(0))
}
...
pub(super) fn exec_wsl2(req: &NativeSandboxRequest) -> Result<NativeSandboxResponse, String> {
    ...
    run_with_timeout(cmd, timeout_ms, "wsl2", req.max_memory_mb.unwrap_or(0))
}
...
pub(super) fn exec_namespace_fallback_v2(
    ...
    if let Some(unshare) = which_tool("unshare") {
        ...
        return run_with_timeout(cmd, timeout_ms, "namespace", ctx.max_memory_mb.unwrap_or(0));
    }
    ...
    run_with_timeout(cmd, timeout_ms, "bare", ctx.max_memory_mb.unwrap_or(0))
}
...
pub(super) fn exec_wsl2_v2(ctx: &SandboxContextV2) -> Result<NativeSandboxResponse, String> {
    ...
    let mut cmd = Command::new(&wsl);
    cmd.args(&args);
    run_with_timeout(cmd, timeout_ms, "wsl2", ctx.max_memory_mb.unwrap_or(0))
}
