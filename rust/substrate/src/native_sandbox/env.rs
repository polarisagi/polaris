...
pub(super) fn build_safe_env(extra: &[String], sandbox_path: &str) -> Vec<(String, String)> {
    ...
    for kv in extra {
        if let Some((k, v)) = kv.split_once('=') {
            // 剔除高危 key（注入向量）与凭据 key（防泄漏）——与 V2 build_env_v2 口径一致，
            // 杜绝 V1 路径把 *_API_KEY / AWS_SECRET_ACCESS_KEY 等原样透传进沙箱（HE-7 纵深）。
            if !danger_list.contains(&k) && !is_credential_key(k) {
                // 找到已有同 key 则替换，否则追加
