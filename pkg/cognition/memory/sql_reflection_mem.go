...
func NewSQLReflectionMem(db *sql.DB) *SQLReflectionMem {
	return &SQLReflectionMem{db: db}
}

// AppendReflection 写入一条反思记录。task_type / salience 等结构字段从 Meta 中提取存入专用列。
func (rm *SQLReflectionMem) AppendReflection(ctx context.Context, entry protocol.ReflectionEntry) error {
	...
	if err != nil {
		return perrors.Wrap(perrors.CodeInternal, "sql_reflection_mem: append", err)
	}
	return nil
}
...
func (rm *SQLReflectionMem) QueryReflections(ctx context.Context, q protocol.ReflectionQuery) ([]protocol.ReflectionEntry, error) {
	...
	if err != nil {
		return nil, perrors.Wrap(perrors.CodeInternal, "sql_reflection_mem: query", err)
	}
	defer rows.Close()

	var results []protocol.ReflectionEntry
	var ids []string
	for rows.Next() {
		...
		if err = rows.Scan(&e.ID, &e.SessionID, &e.AgentID,
			...
			return nil, perrors.Wrap(perrors.CodeInternal, "sql_reflection_mem: scan", err)
		}
	...
	if err = rows.Err(); err != nil {
		return nil, perrors.Wrap(perrors.CodeInternal, "sql_reflection_mem: rows", err)
	}
