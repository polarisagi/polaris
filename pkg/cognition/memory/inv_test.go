...
func Test_inv_M5_01_ImmutableCoreNeverEvicted(t *testing.T) {
	...
	cw.Append(protocol.Message{Role: "system", Content: "immutable-core-sentinel"})

	for i := 0; i < 20; i++ {
		cw.Append(protocol.Message{Role: "user", Content: fmt.Sprintf("filler %d", i)})
	}
...
func Test_inv_M5_01_MultipleSystemMessagesPreserved(t *testing.T) {
	...
	cw.Append(protocol.Message{Role: "system", Content: "identity-block"})
	cw.Append(protocol.Message{Role: "system", Content: "volatile-block"})

	for i := 0; i < 10; i++ {
		cw.Append(protocol.Message{Role: "user", Content: "user message"})
	}
...
func Test_inv_M5_01_OnlySystemRemainsAtZeroBudget(t *testing.T) {
	cw := NewContextWindow(10)
	cw.Append(protocol.Message{Role: "system", Content: "sys"})
	cw.Append(protocol.Message{Role: "user", Content: "u1"})
	cw.Append(protocol.Message{Role: "assistant", Content: "a1"})
