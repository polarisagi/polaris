...
// registeredCommands 是全部已注册斜线命令的权威列表。
var registeredCommands = []slashCommandMeta{
...
type CommandResult struct {
	...
	UpdatedHistory []protocol.Message
}
...
func newSlashCommandRouter(compressor *Compressor, db *sql.DB) *SlashCommandRouter {
	return &SlashCommandRouter{compressor: compressor, db: db}
}
...
func (r *SlashCommandRouter) Dispatch(
	...
	default:
		...
		writeSSEText(w, flusher, msg)
		return CommandResult{Handled: true, Response: msg, UpdatedHistory: history}
	}
}

// handleContext 输出当前上下文 token 使用统计。
func (r *SlashCommandRouter) handleContext(sessionID string, history []protocol.Message, w http.ResponseWriter, flusher http.Flusher) string {
...
func writeSSEText(w http.ResponseWriter, flusher http.Flusher, text string) {
	writeSSE(w, flusher, "token", map[string]string{"content": text})
}
