...
func (s *Server) handleGetVersion(w http.ResponseWriter, _ *http.Request) {
	if s.updater == nil {
		respondJSON(w, updater.VersionInfo{
			Current:      "dev",
			UpdateStatus: updater.StatusIdle,
		})
		return
	}
	respondJSON(w, s.updater.GetVersionInfo())
}
...
func (s *Server) handleTriggerUpdate(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.updater.TriggerUpdate(r.Context(), body.Version); err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	respondJSON(w, map[string]string{"status": "update_started", "version": body.Version})
}
