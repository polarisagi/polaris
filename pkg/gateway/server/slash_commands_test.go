...
func testHistory() []protocol.Message {
...
func TestCompressor_Stats_Threshold(t *testing.T) {
	...
	msgs := []protocol.Message{{Role: "user", Content: repeat("x", 200)}}
	stats := c.Stats(msgs)
...
func TestSplitMessages_LargeHistory(t *testing.T) {
	...
	msgs := make([]protocol.Message, 0, 10)
	for range 10 {
		msgs = append(msgs, protocol.Message{Role: "user", Content: repeat("x", 40)})
	}
