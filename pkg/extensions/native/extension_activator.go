...
func NewExtensionActivator(db *sql.DB, cognitive CognitiveSearcher, mcpMgr *mcp.MCPManager) *ExtensionActivator {
	return &ExtensionActivator{db: db, cognitive: cognitive, mcpMgr: mcpMgr}
}
...
func (a *ExtensionActivator) activateOne(ctx context.Context, extID, snippet string) (*ActivatedToolHint, error) {
	// 查 extension_instances 取类型和运行时元数据
	var extType, runtimeID, config string
	err := a.db.QueryRowContext(ctx, `
		SELECT ext_type, runtime_id, config
		FROM extension_instances
		WHERE id = ? AND status = 'installed'
	`, extID).Scan(&extType, &runtimeID, &config)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil // 未安装或已卸载，跳过
		}
		return nil, perrors.Wrap(perrors.CodeInternal, "activateOne: query failed", err)
	}
...
func (a *ExtensionActivator) activateMCP(ctx context.Context, extID, runtimeID, configJSON, snippet string) (*ActivatedToolHint, error) {
	...
	var command, argsJSON, url, transport string
	err := a.db.QueryRowContext(ctx, `
		SELECT command, args, url, transport
		FROM mcp_servers
		WHERE id = ?
	`, runtimeID).Scan(&command, &argsJSON, &url, &transport)
	if err != nil {
		return nil, perrors.Wrap(perrors.CodeInternal, "activateMCP: query mcp_servers failed", err)
	}
