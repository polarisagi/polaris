...
func makeGitDiffFn(allowedPaths []string) action.InProcessFn {
	return func(ctx context.Context, input []byte) ([]byte, error) {
		...
		if err := json.Unmarshal(input, &args); err != nil {
			return nil, perrors.Wrap(perrors.CodeInternal, "git_diff: invalid args", err)
		}
		if args.Path == "" {
			return nil, perrors.New(perrors.CodeInternal, "git_diff: path is required")
		}
		...
		if args.File != "" {
			...
			if strings.Contains(args.File, "..") {
				return nil, perrors.New(perrors.CodeInternal, "git_diff: file path must not contain '..'")
			}
		...
		if err != nil {
			...
			return nil, perrors.Wrap(perrors.CodeInternal, "git_diff: exec failed", err)
		}
		raw := string(rawOut)
		truncated := false
		if len(raw) > 1<<20 {
...
func makeGitCommitFn(allowedPaths []string) action.InProcessFn {
	return func(ctx context.Context, input []byte) ([]byte, error) {
		var args struct {
		...
		if err := json.Unmarshal(input, &args); err != nil {
			return nil, perrors.Wrap(perrors.CodeInternal, "git_commit: invalid args", err)
		}
		if args.Path == "" {
			return nil, perrors.New(perrors.CodeInternal, "git_commit: path is required")
		}
		if args.Message == "" {
			return nil, perrors.New(perrors.CodeInternal, "git_commit: message is required")
		}
		if err := checkAllowedPath(args.Path, allowedPaths); err != nil {
			return nil, err
		}
		...
		if err != nil {
			return nil, perrors.Wrap(perrors.CodeInternal, "git_commit: rev-parse failed", err)
		}
...
var templateRenderFn action.InProcessFn = func(_ context.Context, input []byte) ([]byte, error) {
	var args struct {
		Template string         `json:"template"`
		Data     map[string]any `json:"data"`
	}
	if err := json.Unmarshal(input, &args); err != nil {
		return nil, perrors.Wrap(perrors.CodeInternal, "template_render: invalid args", err)
	}
	if args.Template == "" {
		return nil, perrors.New(perrors.CodeInternal, "template_render: template is required")
	}
