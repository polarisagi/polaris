...
func (m *Manager) TriggerUpdate(ctx context.Context, version string) error {
	if version == "" {
		return fmt.Errorf("updater: version is required")
	}
	m.mu.RLock()
	...
	if status != StatusIdle && status != StatusError {
		return fmt.Errorf("updater: update already in progress (%s)", status)
	}
...
func (m *Manager) verifyChecksum(ctx context.Context, version, archiveName, archivePath string) error {
	checksumURL := fmt.Sprintf(
		...
		repoOwner, repoName, version, version,
	)
	...
	if !hmac.Equal(actualHash, expectedHash) {
		return fmt.Errorf("SHA-256 mismatch: expected %x, got %x", expectedHash, actualHash)
	}
...
func defaultRestart(exePath string) {
