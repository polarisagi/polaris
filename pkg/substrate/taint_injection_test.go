...
func TestSanitizeToSafe_InjectionBlocked(t *testing.T) {
	...
	ts := NewTaintedString(
		...
		TaintSource{OriginTaintLevel: protocol.TaintMedium},
		"external_file",
	)
...
func TestSanitizeToSafe_InjectionInLowTaint(t *testing.T) {
	...
	ts := NewTaintedString(
		...
		TaintSource{OriginTaintLevel: protocol.TaintLow},
		"internal_config",
	)
...
func TestPipelineTypes(_ *testing.T) {
	...
	vr := protocol.VerificationResult{
		Verdict: protocol.VerdictBlocker,
		Summary: "goal not achieved",
		Findings: []protocol.VerificationFinding{
			{Verdict: protocol.VerdictBlocker, Description: "core feature missing", EvidencePath: "pkg/foo/bar.go"},
		},
	}
