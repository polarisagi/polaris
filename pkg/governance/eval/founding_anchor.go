// Package policy 提供 Polaris 治理策略原语。
...
// trajectories 类型来自 pkg/governance/eval.TrajectoryTrace。
func ComputeFingerprint(trajectories []TrajectoryTrace) BehaviorFingerprint {
...
func LoadOrCreate(dataDir string, privKey ed25519.PrivateKey, trajectories []TrajectoryTrace) (*FoundingAnchor, bool, error) {
...
// ─── OTel 漂移评分原子存储（供 metrics.go RegisterCallback 读取）──────────────

var globalDriftScore atomic.Value // 存储 float64

// SetGlobalDriftScore 更新全局漂移评分（由 BackgroundTaskScheduler 每周调用）。
func SetGlobalDriftScore(score float64) { globalDriftScore.Store(score) }

// GetGlobalDriftScore 读取最新全局漂移评分。零值为 0.0。
func GetGlobalDriftScore() float64 {
	if v := globalDriftScore.Load(); v != nil {
		return v.(float64)
	}
	return 0.0
}

// SelfImproveFreeze 漂移超过 DriftFreezeThreshold 时为 true。
// M9 BackgroundTaskScheduler 每轮检查此标志。
var SelfImproveFreeze atomic.Bool
