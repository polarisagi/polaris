...
func NewHeuristicsStore(db *sql.DB) *HeuristicsStore {
	return &HeuristicsStore{db: db}
}

// LoadHeuristics 启动时从 heuristics_memory 恢复策略到内存 map。
func (hs *HeuristicsStore) LoadHeuristics(ctx context.Context) (map[string][]*PromptStrategy, error) {
	...
	`)
	if err != nil {
		return nil, perrors.Wrap(perrors.CodeInternal, "heuristics_store: load heuristics", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(&s.ID, &taskType, &s.Template, &s.SuccessRate, &s.UseCount); err != nil {
			return nil, perrors.Wrap(perrors.CodeInternal, "heuristics_store: scan heuristic", err)
		}
...
// LoadFallacies 启动时从 fallacy_records 恢复避免规则到内存 map。
func (hs *HeuristicsStore) LoadFallacies(ctx context.Context) (map[string]*ErrorPattern, error) {
	rows, err := hs.db.QueryContext(ctx, `
	...
	`)
	if err != nil {
		return nil, perrors.Wrap(perrors.CodeInternal, "heuristics_store: load fallacies", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(&p.ID, &p.TaskType, &p.AvoidRule, &p.Frequency); err != nil {
			return nil, perrors.Wrap(perrors.CodeInternal, "heuristics_store: scan fallacy", err)
		}
		result[p.ID] = p
	}
	return result, rows.Err()
...
func (hs *HeuristicsStore) SaveHeuristic(ctx context.Context, taskType string, s *PromptStrategy) error {
	if s.ID == "" {
		return perrors.New(perrors.CodeInvalidInput, "heuristics_store: strategy ID empty")
	}
	...
	if err != nil {
		return perrors.Wrap(perrors.CodeInternal, "heuristics_store: save heuristic", err)
	}
	return nil
}
...
func (hs *HeuristicsStore) SaveFallacy(ctx context.Context, p *ErrorPattern) error {
	if p.ID == "" {
		return perrors.New(perrors.CodeInvalidInput, "heuristics_store: pattern ID empty")
	}
	...
	if err != nil {
		return perrors.Wrap(perrors.CodeInternal, "heuristics_store: save fallacy", err)
	}
	return nil
}
