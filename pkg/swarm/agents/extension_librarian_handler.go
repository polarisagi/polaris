...
func (h *ExtensionLibrarianHandler) Handle(ctx context.Context, record *substrate.OutboxRecord) error {
	...
	if err != nil {
		...
		return perrors.Wrap(perrors.CodeInternal, "extension_librarian: query instance failed", err)
	}
...
func (h *ExtensionLibrarianHandler) getInstanceInfo(ctx context.Context, extID string) (string, string, string, error) {
	...
	return name, publisher, installPath, err
}
...
func (h *ExtensionLibrarianHandler) analyzeContent(ctx context.Context, docContent string) (*extAnalysis, error) {
	...
	if err != nil {
		return nil, perrors.Wrap(perrors.CodeInternal, "extension_librarian: llm infer failed", err)
	}

	var parsed extAnalysis
	if err := json.Unmarshal([]byte(descJSON), &parsed); err != nil {
		return nil, perrors.Wrap(perrors.CodeInternal, "extension_librarian: parse llm response failed", err)
	}
	return &parsed, nil
}
...
func (h *ExtensionLibrarianHandler) indexAndRelate(ctx context.Context, extID string, parsed *extAnalysis) error {
	...
	if err := h.surreal.FTSIndex(docID, indexText); err != nil {
		return perrors.Wrap(perrors.CodeInternal, "extension_librarian: FTSIndex failed", err)
	}

	for _, capStr := range parsed.Capabilities {
		if err := h.surreal.GraphRelate("extension:"+extID, "provides", "cap:"+capStr, 1.0); err != nil {
			return perrors.Wrap(perrors.CodeInternal, "extension_librarian: GraphRelate failed", err)
		}
	}
	...
	if h.embedFn != nil {
		...
		if err != nil {
			return perrors.Wrap(perrors.CodeInternal, "extension_librarian: embed failed", err)
		}
		if err := h.surreal.VecUpsert(docID, emb); err != nil {
			return perrors.Wrap(perrors.CodeInternal, "extension_librarian: VecUpsert failed", err)
		}
	}
	...
	`, extID)
	if err != nil {
		return perrors.Wrap(perrors.CodeInternal, "extension_librarian: update meta failed", err)
	}
