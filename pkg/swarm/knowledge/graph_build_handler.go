...
func (h *GraphBuildOutboxHandler) Handle(ctx context.Context, record *substrate.OutboxRecord) error {
	var payload struct {
		DocID string `json:"doc_id"`
	}
	if err := json.Unmarshal(record.Payload, &payload); err != nil {
		return perrors.Wrap(perrors.CodeInvalidInput, "GraphBuildOutboxHandler: invalid payload", err)
	}
	if payload.DocID == "" {
		return nil
	}
