...
func runBenchmarkRouting(args []string) error { //nolint:gocyclo
	if len(args) < 1 {
		return apperr.New(apperr.CodeInvalidInput, "usage: polaris benchmark-routing <promptfoo_results.json> [--help]")
	}

	resultPath := args[0]
	data, err := os.ReadFile(resultPath)
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, fmt.Sprintf("read promptfoo results: %s", resultPath), err)
	}

	var results promptfooResult
	if err := json.Unmarshal(data, &results); err != nil {
		return apperr.Wrap(apperr.CodeInternal, "parse promptfoo results", err)
	}
	...
	fmt.Println("\n========================================")
	fmt.Println("Polaris — M1 Routing Benchmark")
	fmt.Println("========================================")
