...
func migrateMemory(dbPath string, stage bool, smart bool) error {
	...
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "打开 OpenClaw 记忆数据库失败: "+dbPath, err)
	}
	defer ocDB.Close()

	tables, err := introspectTables(ocDB)
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "自省 OpenClaw 数据库 schema 失败", err)
	}
	if len(tables) == 0 {
	...
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "打开 polaris 数据库失败: "+polarisDB, err)
	}
	defer polDB.Close()

	if err := verifyEventsTable(polDB); err != nil {
		return apperr.Wrap(apperr.CodeInternal, "polaris events 表未就绪", err)
	}
	...
	if stage {
		if err := ensureStagingTable(polDB); err != nil {
			return apperr.Wrap(apperr.CodeInternal, "创建 migration_staging 表失败", err)
		}
	}
...
func mapTableWithOpts(ocDB, polDB *sql.DB, table string, opts migrateOpts) (int, error) { //nolint:gocyclo
	...
	if err != nil {
		return 0, apperr.Wrap(apperr.CodeInternal, "自省列信息失败: "+table, err)
	}
	if len(cols) == 0 {
		return 0, nil
	}
	...
	if err != nil {
		return 0, apperr.Wrap(apperr.CodeInternal, "查询表失败: "+table, err)
	}
	defer ocRows.Close()
	...
	for ocRows.Next() {
		...
		if err := ocRows.Scan(ptrs...); err != nil {
			return 0, apperr.Wrap(apperr.CodeInternal, "扫描行数据失败", err)
		}
	...
	insertStmt, err := polDB.Prepare(`
		...
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`)
	if err != nil {
		return 0, apperr.Wrap(apperr.CodeInternal, "准备 events 插入语句失败", err)
	}
