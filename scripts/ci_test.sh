...
echo "[1/9] 准备环境: 创建 Mock Web dist..."
mkdir -p web/dist
touch web/dist/index.html

echo "[2/9] 执行 golangci-lint 静态检查..."
...
echo "[3/9] 执行 docs/arch 一致性检查..."
make docs-check
make docs-lint

echo "[4/9] 编译 Rust Substrate 模块..."
make rust-build

echo "[5/9] 验证 Spec 一致性 (state.yaml SSoT)..."
go test -run "^TestSpec" ./internal/protocol/... -v

echo "[6/9] 运行 Go 全量单元测试 (带竞争检测与覆盖率分析)..."
go test ./pkg/... ./internal/... -v -race -coverprofile=coverage.out
go tool cover -func=coverage.out

echo "[7/9] 运行 Rust 全量单元测试与格式化检查..."
cargo test --manifest-path rust/substrate/Cargo.toml
cargo fmt --manifest-path rust/substrate/Cargo.toml --check

echo "[8/9] 执行全量编译 (make build)..."
make build

echo "[9/9] 验证 Eval Harness Gate..."
