...
type EpisodicGraphIndexer struct {
	graph protocol.GraphTraverser
}

func NewEpisodicGraphIndexer(graph protocol.GraphTraverser) *EpisodicGraphIndexer {
	return &EpisodicGraphIndexer{graph: graph}
}

// Index 为 episodic 事件在图中建立关联边（best-effort：失败仅记日志，不阻断写路径）。
func (ei *EpisodicGraphIndexer) Index(_ context.Context, ev types.Event) {
	node := "episodic:" + ev.ID
	...
	if ev.Type == types.EventActionDone {
