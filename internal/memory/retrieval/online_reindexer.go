...
type OnlineReindexer struct {
	...
	embedder  Embedder
	batchSize int
	cognitive protocol.CognitiveSearcher // Tier1+：SurrealDB HNSW 写入，nil 时仅更新 SQLite BLOB
}

const defaultReindexBatchSize = 50

// NewOnlineReindexer 创建重建索引器。db 和 embedder 均必须非 nil。
func NewOnlineReindexer(db protocol.SQLQuerier, embedder Embedder) *OnlineReindexer {
...
func NewOnlineReindexerWithCognitive(db protocol.SQLQuerier, embedder Embedder, cognitive protocol.CognitiveSearcher) *OnlineReindexer {
	return &OnlineReindexer{
		db:        db,
		embedder:  embedder,
		batchSize: defaultReindexBatchSize,
...
func (r *OnlineReindexer) Run(ctx context.Context) (processed int, remaining bool, err error) {
	...
	rows, queryErr := r.db.QueryContext(ctx,
		`SELECT id, event_uuid, content FROM episodic_events
		 WHERE (embed_model_version = '' OR embed_model_version != ?) AND archived = 0
		 LIMIT ?`,
		version, r.batchSize,
	)
	if queryErr != nil {
