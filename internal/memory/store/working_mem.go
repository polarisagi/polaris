...
// NewWorkingMemWithDB 创建含 SQL 持久化 NotesStore 和 CoreMemory 的 WorkingMem。
func NewWorkingMemWithDB(db protocol.SQLQuerier) *WorkingMem {
	w := NewWorkingMem()
	if db != nil {
		if sqldb, ok := db.(*sql.DB); ok {
			w.notes = NewSQLNotesStore(sqldb)
			w.coreMemory = NewSQLCoreMemoryStore(sqldb)
		} else {
			// 如果不是 *sql.DB，可能是事务，这里简单处理 NotesStore
...
func (w *WorkingMem) CoreMemory() protocol.CoreMemory   { return w.coreMemory }
