...
func NewReflectionMem(store protocol.Store) *ReflectionMem {
	return &ReflectionMem{
		store:   store,
		entries: make([]types.ReflectionEntry, 0),
	}
}

func (rm *ReflectionMem) AppendReflection(ctx context.Context, entry types.ReflectionEntry) error {
	rm.mu.Lock()
	defer rm.mu.Unlock()
...
func (rm *ReflectionMem) ListReflections( //nolint:gocyclo
	...
	var results []types.ReflectionEntry //nolint:prealloc
	now := time.Now()
	for i, e := range rm.entries {
