...
//
// 来源优先级（高到低）：
//  1. 显式环境变量 POLARIS_LANG / POLARIS_LOCALE
//  2. OS 系统层探测 (macOS defaults / Linux locale)
//  3. 本地缓存（写入 DB，避免每次启动都探测）
//
...
func probeGeoIP(ctx context.Context, baseClient *http.Client) (country string, ok bool) {
	...
	// 探测 1: ipinfo.io
	wg.Add(1)
	concurrent.SafeGo(probeCtx, "sysmgr.locale.probe_ipinfo", func(context.Context) {
		defer wg.Done()
	...
	wg.Add(1)
	concurrent.SafeGo(probeCtx, "sysmgr.locale.probe_cloudflare", func(context.Context) {
		defer wg.Done()
	...
	// 探测 3: api.ip.sb
	wg.Add(1)
	concurrent.SafeGo(probeCtx, "sysmgr.locale.probe_ipsb", func(context.Context) {
		defer wg.Done()
	...
	concurrent.SafeGo(probeCtx, "sysmgr.locale.probe_wait_close", func(context.Context) {
		wg.Wait()
		close(resCh)
	})
