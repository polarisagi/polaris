...
func probeLinuxGPU() GPUInfo {
	...
	probes := []probe{
		{"nvidia-smi", []string{"--query-gpu=name,memory.total", "--format=csv,noheader,nounits"}, "NVIDIA"},
		{"rocm-smi", []string{"--showproductname"}, "AMD"},
		{"xpu-smi", []string{"discovery"}, "Intel"},
	}
	for _, p := range probes {
		ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
		out, err := exec.CommandContext(ctx, p.cmd, p.args...).Output()
		cancel()
		if err != nil {
			continue
		}
		...
		return GPUInfo{Available: true, Vendor: p.name, Name: line}
	}
	return GPUInfo{Available: false}
}
...
type OSMemoryGuard struct {
	...
	slopeWindow         [4]uint64 // 最近 4 次采样的环形缓冲区；slope 取相邻两次（5s delta）
...
func (g *OSMemoryGuard) CheckAndProtect(availableMB uint64) DegradationLevel {
	// 更新斜率环形缓冲区
	// prevIndex 指向最近一次写入的槽位（slopeIndex-1，环绕取模）
	prevIndex := (g.slopeIndex + 3) % 4
	prev := g.slopeWindow[prevIndex]
