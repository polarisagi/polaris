...
// getEntityPatterns 返回实体提取正则模式（懒加载，仅编译一次）。
var getEntityPatterns = sync.OnceValue(func() []*regexp.Regexp {
...
// classifyPatternsT 分类正则集合。
type classifyPatternsT struct {
	version, domain, pascal *regexp.Regexp
}

// getClassifyPatterns 返回实体类型分类正则（懒加载，仅编译一次）。
var getClassifyPatterns = sync.OnceValue(func() classifyPatternsT {
	return classifyPatternsT{
		version: regexp.MustCompile(`^\d+\.\d+\.\d+$`),
		domain:  regexp.MustCompile(`^[\w-]+\.[\w-]+`),
		pascal:  regexp.MustCompile(`^[A-Z][a-z]+(?:[A-Z][a-z]+)+$`),
	}
})
...
func extractEntitiesByPattern(text string) []*Entity {
	...
	var entities []*Entity
	for _, pat := range getEntityPatterns() {
...
func classifyEntityType(name string) string {
	p := getClassifyPatterns()
	if strings.Contains(name, "/") {
		return "path"
	}
	if p.version.MatchString(name) {
		return "version"
	}
	if p.domain.MatchString(name) {
		return "domain"
	}
	if strings.ContainsAny(name, "._") || p.pascal.MatchString(name) {
		return "identifier"
	}
	return "concept"
}
