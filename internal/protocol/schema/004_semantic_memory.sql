...
CREATE TABLE IF NOT EXISTS semantic_entities (
    ...
    -- ↑ 生命周期状态: 'active' | 'superseded' | 'expired' | 'merged' | 'pending_review'。
    ...
    -- ↑ 事实置信度 0.0-1.0。
    --   llm_extract=0.8，rule_extract=0.6，user_stated=1.0，agent_inferred=0.7。
    --   HybridRetriever RRF 加权时乘以此值。

    source_type     TEXT    NOT NULL DEFAULT 'llm_extract',
    -- ↑ 事实来源类型: 'llm_extract' | 'rule_extract' | 'user_stated' | 'agent_inferred'。
    --   memory_write 工具写入时填 'user_stated'。

    valid_from      INTEGER,
    -- ↑ 事实生效起始时间（Unix 毫秒）。NULL = 从创建即生效。
    --   Zep/Graphiti 时态知识图谱模式：事实有有效时间窗。
...
CREATE TABLE IF NOT EXISTS semantic_relations (
    ...
    -- ↑ 最后更新时间（Unix 毫秒），SynapticPlasticity LTP/LTD 更新时填写。
...
-- 时态有效窗索引（TemporalExpirer 扫描加速）
CREATE INDEX IF NOT EXISTS idx_semantic_ent_valid_until
    ON semantic_entities(valid_until)
    WHERE valid_until IS NOT NULL AND status = 'active';

-- 置信度复合索引（HybridRetriever 高置信度优先排序）
CREATE INDEX IF NOT EXISTS idx_semantic_ent_confidence
    ON semantic_entities(status, confidence DESC)
    WHERE status = 'active';

-- 待复核实体索引（人工复核 / 自动重提取队列）
CREATE INDEX IF NOT EXISTS idx_semantic_ent_pending
    ON semantic_entities(status, updated_at)
