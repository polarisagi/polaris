...
CREATE TABLE IF NOT EXISTS automations (
    ...
    -- [Task 19] env_type: 执行环境模式。
    -- 'inline'=在当前工作目录直接执行（原有行为）。
    -- 'worktree'=创建独立 git worktree 分支执行，完成后自动开 PR（代码改动隔离）。
    env_type          TEXT    NOT NULL DEFAULT 'inline' CHECK(env_type IN ('inline', 'worktree')),
    ...
    requires_hitl     BOOLEAN NOT NULL DEFAULT FALSE,    -- 是否需人工审批
    risk_level        INTEGER NOT NULL DEFAULT 1,        -- 风险等级
    -- 执行状态（cronTick 更新）
