...
%s`, replanCount, formattedTraj)
		...
		docID := "her_" + taskID
		if re.surreal != nil {
			if err := re.surreal.FTSIndex(docID, parsed.Insight+" "+fmt.Sprint(parsed.Tags)); err != nil {
				slog.Warn("reflexion: failed to persist reflection memory", "err", err, "context", "surreal_fts_index")
			}
			if err := re.surreal.GraphRelate("task_type:"+taskType, "learned_pattern", docID, float64(replanCount)); err != nil {
				slog.Warn("reflexion: failed to persist reflection memory", "err", err, "context", "surreal_graph_relate")
			}
		}

		if re.db != nil {
			if _, err := re.db.ExecContext(ctx, `
				INSERT OR IGNORE INTO reflection_memory 
				  (task_id, reflection_type, content, created_at)
				VALUES (?, 'success_pattern', ?, ?)
			`, taskID, insightJSON, time.Now().Unix()); err != nil {
				slog.Warn("reflexion: failed to persist reflection memory", "err", err, "context", "episodic_write")
			}
		}
	})
