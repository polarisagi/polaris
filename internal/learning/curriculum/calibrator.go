...
func (ddc *DynamicDifficultyCalibrator) Calibrate() {
	...
	if rate < 0.5 {
		ddc.currentLow = math.Max(0.1, ddc.currentLow-ddc.adjustStep)
		ddc.currentHigh = math.Max(0.1, ddc.currentHigh-ddc.adjustStep)
	} else if rate > 0.7 {
		ddc.currentLow = math.Min(0.85, ddc.currentLow+ddc.adjustStep)
		ddc.currentHigh = math.Min(0.85, ddc.currentHigh+ddc.adjustStep)
	}
}
