...
func (b *Bootstrapper) Ignite(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("bootstrap: dependency resolution failed: %w", err)
	}
	...
	for _, name := range order {
		...
		if err := mod.Init(b.deps); err != nil {
			return fmt.Errorf("bootstrap: module %s init failed: %w", name, err)
		}
		if !mod.Ready() {
			return fmt.Errorf("bootstrap: module %s not ready after init", name)
		}
...
func (b *Bootstrapper) topologicalSort() ([]string, error) {
	...
	for name, mod := range b.modules {
		for _, dep := range mod.Dependencies() {
			if _, exists := b.modules[dep]; !exists {
				return nil, fmt.Errorf("module %s requires missing dependency %s", name, dep)
			}
	...
	if len(order) != len(b.modules) {
		return nil, fmt.Errorf("circular dependency detected among registered modules")
	}
	return order, nil
}
