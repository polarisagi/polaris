...
func NewRateLimiterMiddleware(ctx context.Context) *RateLimiterMiddleware {
	...
	concurrent.SafeGo(ctx, "cli.rate_limiter_cleanup_loop", func(ctx context.Context) {
		r.cleanupLoop(ctx)
	})
	return r
}
...
func (rlm *RateLimiterMiddleware) cleanupLoop(ctx context.Context) {
...
func (hub *WebSocketHub) Run(ctx context.Context) {
	concurrent.SafeGo(ctx, "cli.websocket_hub_run_loop", func(ctx context.Context) {
		for {
			select {
			case <-ctx.Done():
				return
