...
func (h *SysAdminHandler) HandleInsights(w http.ResponseWriter, r *http.Request) { //nolint:gocyclo
	days := 30
	...
	h.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM chat_sessions`).Scan(&totalSessions) //nolint:errcheck
	h.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM chat_messages`).Scan(&totalMessages) //nolint:errcheck
