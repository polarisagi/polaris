...
func allowedUserPrompts() map[string]string {
...
func (h *SysAdminHandler) HandleSetPrompt(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.PromptMgr.WriteUserPrompt(filename, req.Value); err != nil {
...
func (h *SysAdminHandler) HandleResetPrompt(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.PromptMgr.DeleteUserPrompt(filename); err != nil {
	...
	// 热更新：恢复到内置默认
	if name == "identity" {
		(*h.SoulMDContent) = h.PromptMgr.LoadSoulMD() // 重新走三层加载（文件删除后回落到 embedded）
	}
