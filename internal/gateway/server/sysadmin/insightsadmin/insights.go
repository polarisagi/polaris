...
type InsightsAdmin struct {
	DB protocol.SQLQuerier
}

func NewInsightsAdmin(db protocol.SQLQuerier) *InsightsAdmin {
	return &InsightsAdmin{DB: db}
}

// GET /v1/insights?days=30
// 聚合对话历史用量统计：会话数、消息数、每日活跃趋势、角色分布。
// 纯 SQLite 聚合，无 LLM 调用，响应 < 10ms。
func (a *InsightsAdmin) HandleInsights(w http.ResponseWriter, r *http.Request) { //nolint:gocyclo
	days := 30
	...
	a.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM chat_sessions`).Scan(&totalSessions) //nolint:errcheck
	a.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM chat_messages`).Scan(&totalMessages) //nolint:errcheck
	...
	// ── 角色分布 ──────────────────────────────────────────────────────────────
	roleRows, err := a.DB.QueryContext(ctx,
