...
func NewRunner(registry *Registry, policy protocol.PolicyGate, envelope *sandbox.ExecEnvelope, piiDetector *guard.PIIDetector) *Runner {
	if piiDetector == nil {
		piiDetector = guard.NewPIIDetector()
	}
	return &Runner{registry: registry, policy: policy, envelope: envelope, piiDetector: piiDetector}
}
...
func (r *Runner) redactHookInput(ctx context.Context, input HookInput) ([]byte, error) {
	...
	redacted, n, err := r.piiDetector.Redact(ctx, string(raw))
	if err != nil {
