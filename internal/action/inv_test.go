...
func Test_inv_M7_04_DelegationChainMaxDepth(t *testing.T) {
	dummyToken, _ := NewJITToken("agent-root", "session-root", []TokenOperation{{ToolName: "bash"}}, 0, 1)

	// parentDepth=3 → 直接被 ValidateDelegation 检查拦截
	_, err := ValidateDelegation(dummyToken, 3, "agent-a", "session-1", nil, 1)
	...
	_, err = ValidateDelegation(dummyToken, 5, "agent-a", "session-1", nil, 1)
	if !errors.Is(err, ErrMaxDelegationDepth) {
		t.Errorf("inv_M7_04: parentDepth=5 got %v, want ErrMaxDelegationDepth", err)
	}

	// parentDepth=100 → 边界极限
	_, err = ValidateDelegation(dummyToken, 100, "agent-a", "session-1", nil, 1)
	if !errors.Is(err, ErrMaxDelegationDepth) {
...
func Test_inv_M7_04_BoundaryAtExactlyDepthThree(t *testing.T) {
	...
	_, err := ValidateDelegation(dummyToken, 2, "agent-b", "session-2", nil, 1)
	if !errors.Is(err, ErrMaxDelegationDepth) {
