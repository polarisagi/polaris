...
// XvfbAvailable 探测本机是否具备 XvfbDisplayServer 运行所需的三个外部二进制
// （xdotool/xwd/convert）。
//
// 2026-07-04 审计修复（Task 3 附录）：原实现仅以 FeatureVisionDisplayServer 的
// OS/Tier/内存三项条件决定是否装配 XvfbDisplayServer，从未探测这三个二进制
// 是否真的安装——Linux Tier1+ 但未装 xdotool 等工具的机器上，Feature 仍会报
// Enabled，SendAction/GetFrame 会在运行时对每次 GUI 动作请求报错，而不是任务
// 文档要求的"启动期优雅降级为 nil no-op + 诊断日志"。调用方应在 FeatureGate
// 判断之外再叠加此探测，作为组合调用方（cmd/polaris/boot_agent.go）的启动期
// 前置条件，而非塞进通用的 FeatureGate.computeState（那里只做 OS/Tier/内存
// 这类硬件/环境探测，二进制可用性属于本模块自己的职责范围）。
func XvfbAvailable() bool {
	for _, bin := range [...]string{"xdotool", "xwd", "convert"} {
