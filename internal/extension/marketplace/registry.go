...
func (r *Registry) Register(p *Plugin) error {
	...
	if _, exists := r.plugins[p.Manifest.Name]; exists {
		return apperr.New(apperr.CodeInternal, fmt.Sprintf("plugin: %q already registered; unregister first", p.Manifest.Name))
	}
...
func (r *Registry) SetEnabled(name string, enabled bool) error {
	...
	p, ok := r.plugins[name]
	if !ok {
		return apperr.New(apperr.CodeInternal, fmt.Sprintf("plugin: %q not found", name))
	}
	p.Enabled = enabled
...
func (r *Registry) ScanDir(dir string) (int, error) {
	...
	if err != nil {
		...
		return 0, apperr.Wrap(apperr.CodeInternal, fmt.Sprintf("plugin: scan %s: %v", dir, err), err)
	}
...
// dataDir 为 DataLayout.Root（~/.polarisagi/polaris），用户安装路径为 dataDir/extensions。
