...
func NewPromptVersionStore(db protocol.SQLQuerier) *PromptVersionStore {
...
func (s *PromptVersionStore) Save(ctx context.Context, v *PromptVersion) error {
	if v.ID == "" {
		return apperr.New(apperr.CodeInvalidInput, "prompt_version_store: version ID must not be empty")
	}
	...
	`, v.ID, v.Version, v.TaskType, v.Prompt, v.Score, v.Cost, v.Source, v.ParentVer,
		time.Now().Unix())
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "prompt_version_store: save failed", err)
	}
	return nil
}
...
func (s *PromptVersionStore) GetActive(ctx context.Context, taskType string) (*PromptVersion, error) {
	...
	if err != nil {
		return nil, apperr.Wrap(apperr.CodeInternal, "prompt_version_store: get active failed", err)
	}
	v.Active = true
	return v, nil
}
...
func (s *PromptVersionStore) UpdateScore(ctx context.Context, id string, score float64) error {
	...
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "prompt_version_store: update score failed", err)
	}
	rows, _ := res.RowsAffected()
	if rows == 0 {
		return apperr.New(apperr.CodeNotFound, fmt.Sprintf("prompt_version_store: version %q not found", id))
	}
	return nil
}
...
func (s *PromptVersionStore) Activate(ctx context.Context, taskType, id string, baselineScore float64) error {
	...
	if !ok {
		return apperr.New(apperr.CodeInternal, "prompt_version_store: db does not support transactions")
	}
	tx, err := starter.BeginTx(ctx, nil)
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "prompt_version_store: begin tx failed", err)
	}
	...
	err = tx.QueryRowContext(ctx,
		...
		Scan(&candidateScore)
	if errors.Is(err, sql.ErrNoRows) {
		return apperr.New(apperr.CodeNotFound, fmt.Sprintf("prompt_version_store: candidate %q not found", id))
	}
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "prompt_version_store: read candidate score failed", err)
	}
	...
	if _, err = tx.ExecContext(ctx,
		...
		return apperr.Wrap(apperr.CodeInternal, "prompt_version_store: deactivate old failed", err)
	}
	// 激活候选
	var promptText string
	if err = tx.QueryRowContext(ctx,
		"SELECT prompt_text FROM prompt_versions WHERE id = ?", id).Scan(&promptText); err != nil {
		return apperr.Wrap(apperr.CodeInternal, "prompt_version_store: read prompt text failed", err)
	}
	if _, err = tx.ExecContext(ctx,
		"UPDATE prompt_versions SET is_active = 1 WHERE id = ?", id); err != nil {
		return apperr.Wrap(apperr.CodeInternal, "prompt_version_store: activate failed", err)
	}
	if err = tx.Commit(); err != nil {
		return apperr.Wrap(apperr.CodeInternal, "prompt_version_store: commit failed", err)
	}
...
func (s *PromptVersionStore) ListRecent(ctx context.Context, taskType string, n int) ([]*PromptVersion, error) {
	...
	`, taskType, n)
	if err != nil {
		return nil, apperr.Wrap(apperr.CodeInternal, "prompt_version_store: list recent failed", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err = rows.Scan(&v.ID, &v.Version, &v.TaskType, &v.Prompt,
			...
			return nil, apperr.Wrap(apperr.CodeInternal, "prompt_version_store: scan failed", err)
		}
		v.Active = isActive == 1
