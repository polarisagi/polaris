...
func (s *Supervisor) runWorker(w *WorkerEntry) {
	...
	for {
		...
		if currentCount > s.maxRestarts {
			// 记录真实崩溃原因（GR-7.1-006）：原实现用占位 error 替换，升级告警无法定位根因
			slog.Error("supervisor: worker exceeded max restarts, escalating", "worker", w.ID, "max_restarts", s.maxRestarts, "err", err)
...
func (s *Supervisor) executeSafely(w *WorkerEntry) (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = apperr.New(apperr.CodeInternal, fmt.Sprintf("panic: %v", r))
		}
	}()
	err = w.Fn(s.ctx)
	if err != nil {
