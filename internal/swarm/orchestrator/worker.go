...
func (w *Worker) ListenLoop(ctx context.Context) error {
	...
	for {
		...
		case ev, ok := <-events:
			if !ok {
				return apperr.New(apperr.CodeInternal, "blackboard event channel closed")
			}
...
func (w *Worker) tryClaimAndExecute(ctx context.Context, taskID string) {
	...
	for {
		...
		case <-done:
			...
			// 根据结果写回 Blackboard
			if finalState == types.AgentStateFailed {
				slog.Error("worker: task failed", "agent", w.agentID, "task_id", taskID, "err", apperr.New(apperr.CodeInternal, "log event"))
