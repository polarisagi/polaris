...
type PipelineOrchestrator struct {
	...
	hitl         protocol.HITL
	decisionLog  protocol.DecisionLogger
	pollInterval time.Duration
	compTimeout  time.Duration
	compPoll     time.Duration
}
...
func NewPipelineOrchestrator(
	...
	compTimeout := 5 * time.Minute
	if compTimeoutSec > 0 {
		compTimeout = time.Duration(compTimeoutSec) * time.Second
	}
	compPoll := 5 * time.Second
	if compPollSec > 0 {
		compPoll = time.Duration(compPollSec) * time.Second
	}
