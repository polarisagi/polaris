...
func RunCSVFanout(ctx context.Context, bb protocol.Blackboard, job CSVFanoutJob) (*FanoutResult, error) { //nolint:gocyclo
	...
	if err != nil {
		return nil, apperr.Wrap(apperr.CodeInternal, fmt.Sprintf("csv_fanout: read %s: %v", job.CSVPath, err), err)
	}
	...
	entries := make([]*types.TaskEntry, 0, len(rows))
	...
	for i, entry := range entries {
		wg.Add(1)
		sem <- struct{}{}
		go func(idx int, e *types.TaskEntry) {
			defer wg.Done()
			defer func() { <-sem }()
			...
			if job.EventLog != nil {
				...
				_ = job.EventLog.Append(waitCtx, types.Event{
					ID:      e.ID + "_result",
	...
	if job.OutputCSVPath != "" {
		if err := writeResultCSV(job.OutputCSVPath, headers, results); err != nil {
			return fanout, apperr.Wrap(apperr.CodeInternal, fmt.Sprintf("csv_fanout: write output: %v", err), err)
		}
	}
	return fanout, nil
}
