...
type MemoryAgent struct {
	db              protocol.SQLQuerier    // SQLite，读 episodic_memory
...
func NewMemoryAgent(db protocol.SQLQuerier, surreal SurrealWriterInterface, llmInfer LLMInferFunc,
...
func (ma *MemoryAgent) distill(ctx context.Context) error {
	...
	now := time.Now().Unix()
	for _, id := range ids {
		jsonPatch := fmt.Sprintf(`{"distilled_at": %d}`, now)
		_, _ = ma.db.ExecContext(distillCtx, `
			UPDATE episodic_memory SET meta = json_patch(meta, ?)
			WHERE id = ?
		`, jsonPatch, id)
	}
