...
// 使用方式：
//
//	tracker := llm.NewRateLimitTracker()
//	transport := &llm.RateLimitCapturingTransport{
//	    Inner:    http.DefaultTransport,
//	    Tracker:  tracker,
//	    Provider: "anthropic",
//	}
//	client := &http.Client{Transport: transport}
//	adapter := llm.NewAnthropicAdapter(model, credFn, client)
...
func (c BackoffConfig) Delay(attempt int) time.Duration {
	...
	// 去相关种子：math/rand/v2 默认全局源是高并发安全的 ChaCha8/PCG
	jitter := time.Duration(float64(exp) * jr * rand.Float64())
