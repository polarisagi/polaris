...
// 已被 llm.Classify + TierFromInt 替代，保留供旧调用方使用。
...
// TierFromInt 将 llm.ClassifiedError.FallbackTierInt() 的返回值转为 FallbackTier。
// 解耦两个包，调用方不需要同时 import substrate 和 substrate/inference。
//
// 用法：
//
//	ce := llm.ClassifyWithProvider(err, "anthropic")
...
func (fe *FallbackExecutor) Execute() error {
	if !fe.breaker.Allow() {
		return apperr.New(apperr.CodeInternal, "circuit breaker open — all providers throttled")
	}
