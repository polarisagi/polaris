...
func (a *SteeringAdapter) SteerActivations(ctx context.Context, req *SteerRequest) (*SteerResponse, error) {
	if req == nil || len(req.Vector) == 0 {
		return nil, apperr.New(apperr.CodeInvalidInput, "steering vector is empty")
	}

	body, err := json.Marshal(req)
	if err != nil {
		return nil, apperr.Wrap(apperr.CodeInternal, "marshal steer req", err)
	}

	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, a.endpoint, bytes.NewReader(body))
	if err != nil {
		return nil, apperr.Wrap(apperr.CodeInternal, "build steer req", err)
	}
	...
	if err != nil {
		...
		return nil, apperr.Wrap(apperr.CodeInternal, "steer http", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode >= 400 {
		raw, _ := io.ReadAll(io.LimitReader(resp.Body, 1024))
		return nil, apperr.New(apperr.CodeInternal, fmt.Sprintf("steer service error %d: %s", resp.StatusCode, raw))
	}

	var out SteerResponse
	if err := json.NewDecoder(resp.Body).Decode(&out); err != nil {
		return nil, apperr.Wrap(apperr.CodeInternal, "decode steer resp", err)
	}
...
func (a *SteeringAdapter) ClearSteering(ctx context.Context, sessionID string) error {
	...
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "build clear steer req", err)
	}
	resp, err := a.client.Do(req)
	if err != nil {
		return apperr.Wrap(apperr.CodeInternal, "clear steer http", err)
	}
	defer resp.Body.Close()
	return nil
}
