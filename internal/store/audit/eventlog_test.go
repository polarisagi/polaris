...
func TestEventLogger(t *testing.T) {
	...
	// store.OpenSQLite 会返回 *SQLiteStore，里面的 db 可以通过暴露出 GetDB 或者直接强制转换，或者我们在 store.go 里面加一个 DB() 方法。
	...
	// 验证落盘
	var count int
	if err := testStore.DB().QueryRow("SELECT COUNT(*) FROM events WHERE id = ?", ev.Id).Scan(&count); err != nil {
