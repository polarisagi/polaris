...
// candidateVersion: 当前测试版本；candidateOpts: 该版本携带的新 Prompt/权重等覆盖参数。
func (e *ShadowExecutor) RunReplayBatch(ctx context.Context, candidateVersion string, candidateOpts []types.InferOption) error {
	...
	for _, s := range samples {
		...
		e.processSingleSample(ctx, s, candidateOpts, &rm)
	}
...
func (e *ShadowExecutor) processSingleSample(ctx context.Context, s sampleData, candidateOpts []types.InferOption, rm *ReplayMetrics) {
