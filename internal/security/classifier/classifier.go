...
// NewDefaultClassifier 构造内置默认规则集分级器。
// 依赖注入替代包级可变变量（R1.3）：本函数每次调用返回全新实例（正则编译，
// 非零成本），调用方若处于热路径（每次工具调用都要分级）应在自己的构造函数/
// 工厂函数里调用一次并缓存实例复用，不要在请求路径里反复调用本函数；
// 是否缓存、缓存在哪一层，由调用方根据自身生命周期决定,不由本包替调用方决定。
func NewDefaultClassifier() *CommandRiskClassifier {
