...
func TestTaintGate_SlotViolation(t *testing.T) {
	...
	if err := gate.CheckSlotAssignment(taint.SlotInstruction, types.TaintMedium); err == nil {
...
// TestCapabilityToken_MintVerify 验证 Capability token.Token 签发与验证。
func TestCapabilityToken_MintVerify(t *testing.T) {
	tm, err := token.NewTokenManager()
	if err != nil {
		t.Fatalf("token.NewTokenManager: %v", err)
	}

	tok, err := tm.Mint("agent-0", []token.CapabilityType{token.CapNetwork}, 1, 0, 0)
	if err != nil {
		t.Fatalf("Mint: %v", err)
	}
...
func TestCapabilityToken_Revoke(t *testing.T) {
	...
	tok, _ := tm.Mint("agent-0", []token.CapabilityType{token.CapShell}, 1, 0, 0)

	tm.Revoke(tok.Claims.TokenID)

	if err := tm.Verify(tok); err != token.ErrTokenRevoked {
...
func TestGate_ForbidLLMGeneratedPrivilege(t *testing.T) {
	gate := policy.NewGate(nil).WithEvalTimeout(2 * time.Second)
	ctx := context.Background()
