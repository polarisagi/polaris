...
// knownInjectionPatterns 覆盖 OWASP LLM01 常见间接注入手法（只读）。
// 保持简短（无正则）：高置信度匹配优先于覆盖率，减少假阳性。
func knownInjectionPatterns() []injectionPattern {
...
func SanitizeBySchema(ts TaintedString, hasStrictSchema bool) (TaintedString, error) {
	if !hasStrictSchema {
		...
		return ts, apperr.New(apperr.CodeInvalidInput, "policy: strict schema (format/pattern/enum) required for sanitization")
	}
...
func SanitizeByUserReview(ts TaintedString, reviewerID string) TaintedString {
	ts.Source.OriginTaintLevel = types.TaintUserReviewed
...
// 结果: data.Level = min(Level-1, TaintMedium)
//
// [保留说明] M11 §2.5 四降级器之一，ADR-0007 §决策 3 明确"刻意保持不接入 S_VALIDATE"：
