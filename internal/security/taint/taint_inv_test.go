...
func Test_inv_M11_02_TaintLevelOnlyRises(t *testing.T) {
	...
	tt.Track("id1", types.TaintMedium)
	if tt.GetLevel("id1") != types.TaintHigh {
...
func Test_inv_M11_02_MaxTaintEqualsMaxInput(t *testing.T) {
	tt := NewTaintTracker()
	tt.Track("a", types.TaintLow)
	tt.Track("b", types.TaintUserReviewed) // TaintUserReviewed(4) 是最高值
	tt.Track("c", types.TaintMedium)
...
func Test_inv_M11_02_TaintBoundaryHMACMismatchUpgradesToHigh(t *testing.T) {
	...
	if recovered.Level() != types.TaintHigh {
