...
func TestGate_CheckEgressWithExemption(t *testing.T) {
	...
	// 2. Taint medium without token -> policy.ErrTaintBlockedEgress
	if err := gate.CheckEgressWithExemption(data, types.TaintMedium, nil); err != policy.ErrTaintBlockedEgress {
	...
	if err := gate.CheckEgressWithExemption(data, types.TaintMedium, invalidTok); err != policy.ErrTaintBlockedEgress {
		t.Errorf("Expected policy.ErrTaintBlockedEgress, got %v", err)
	}
}
