...
func TestAuditTrail_Record_WritesToDB(t *testing.T) {
	db := openTestDB(t)
	at := NewAuditTrail(repo.NewSQLiteAuditRepository(db), t.TempDir())
...
func TestAuditTrail_RecoverOnStartup_ReloadsFromDB(t *testing.T) {
	...
	// Create some records
	at1 := NewAuditTrail(repo.NewSQLiteAuditRepository(db), dir)
	...
	at2 := NewAuditTrail(repo.NewSQLiteAuditRepository(db), dir)
