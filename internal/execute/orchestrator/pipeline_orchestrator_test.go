...
func TestPipelineOrchestrator_SingleStage_Pass(t *testing.T) {
	...
	po := NewPipelineOrchestrator(bb, nil, nil, 10*time.Millisecond, 0, 0)
...
// --- Compensation stub/tests ---

type stubHITL struct {
	prompts []types.HITLPrompt
}

func (s *stubHITL) Prompt(ctx context.Context, p types.HITLPrompt) (*types.HITLResponse, error) {
	s.prompts = append(s.prompts, p)
	return nil, nil
}
func (s *stubHITL) Respond(ctx context.Context, checkpointID string, response types.HITLResponse) error {
	return nil
}
func (s *stubHITL) Pending(ctx context.Context) ([]types.HITLPrompt, error) {
	return s.prompts, nil
}

type stubDecisionLogger struct {
	entries []*types.DecisionLogEntry
}

func (s *stubDecisionLogger) AppendDecision(ctx context.Context, entry *types.DecisionLogEntry) error {
	s.entries = append(s.entries, entry)
	return nil
}

func TestPipelineOrchestrator_CompensationEscalation(t *testing.T) {
	bb := newStubBlackboard(map[string]taskOutcome{
		"comp-pipe-research-compensate": {status: types.TaskFailed},
	})

	hitl := &stubHITL{}
	dlog := &stubDecisionLogger{}

	po := NewPipelineOrchestrator(bb, hitl, dlog, 10*time.Millisecond, 1, 1)
	po.compPoll = 10 * time.Millisecond
	po.compTimeout = 50 * time.Millisecond

	ctx := context.Background()
	po.monitorCompensationTask(ctx, "comp-pipe-research-compensate", "research", "comp-pipe")

	if len(hitl.prompts) == 0 {
		t.Error("expected HITL escalation prompt to be triggered")
	} else if hitl.prompts[0].RiskLevel != 3 {
		t.Errorf("expected RiskLevel 3, got %d", hitl.prompts[0].RiskLevel)
	}

	if len(dlog.entries) == 0 {
		t.Error("expected DecisionLog entry to be created")
	} else if dlog.entries[0].Choice != "ESCALATE" {
		t.Errorf("expected Decision choice ESCALATE, got %s", dlog.entries[0].Choice)
	}

	bbTimeout := newStubBlackboard(nil)
	hitlTimeout := &stubHITL{}
	dlogTimeout := &stubDecisionLogger{}
	poTimeout := NewPipelineOrchestrator(bbTimeout, hitlTimeout, dlogTimeout, 10*time.Millisecond, 1, 1)
	poTimeout.compPoll = 10 * time.Millisecond
	poTimeout.compTimeout = 50 * time.Millisecond

	poTimeout.monitorCompensationTask(ctx, "comp-pipe-research-compensate-2", "research", "comp-pipe")

	if len(hitlTimeout.prompts) == 0 {
		t.Error("expected HITL escalation prompt to be triggered on timeout")
	}
	if len(dlogTimeout.entries) == 0 {
		t.Error("expected DecisionLog entry to be created on timeout")
	}
}
